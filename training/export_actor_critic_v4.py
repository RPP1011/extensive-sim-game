#!/usr/bin/env python3
"""Export V4 dual-head actor-critic weights to JSON for Rust inference.

V4 has two heads:
  - Move head: 9-way directional classification (8 cardinal + stay)
  - Combat head: type (attack/hold/ability) + pointer for targeting

Usage:
    uv run --with numpy --with torch training/export_actor_critic_v4.py \
        generated/actor_critic_v4_p3.pt \
        -o generated/actor_critic_weights_v4.json \
...
def main():
    p = argparse.ArgumentParser(description="Export V4 dual-head actor-critic weights to JSON")
    p.add_argument("checkpoint", help="V4 actor-critic checkpoint (.pt)")
    p.add_argument("-o", "--output", default="generated/actor_critic_weights_v4.json")
    ...
    # Combat pointer head (attack/hold/ability + pointer)
    export["combat_head"] = {
        "combat_type_head": {
