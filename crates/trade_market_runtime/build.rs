//! `trade_market_runtime` build script. Mirrors `auction_runtime`'s
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/trade_market_runtime");
    let sim_path = workspace_root.join("assets/sim/trade_market_probe.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse trade_market_probe.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve trade_market_probe.sim");
    let cg = dsl_compiler::cg::lower::lower_compilation_to_cg(&comp)
        .expect("lower trade_market_probe.sim to CG");
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit trade_market CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[trade_market emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[trade_market emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by trade_market_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/trade_market_probe.sim and rebuilding.\n\n",
    );
