...
/// @materialized view `rally_boost` — `storage = per_entity_topk(K=8)`.
...
impl RallyBoost {
    ...
    /// Current value for `wounded_kin` as observed by `observer`,
    /// decayed from its anchor tick. Returns `initial` when the pair isn't in observer's top-8.
    pub fn get(&self, observer: AgentId, wounded_kin: AgentId, tick: u32) -> f32 {
        let obs_slot = (observer.raw() - 1) as usize;
        let atk_id = wounded_kin.raw();
    ...
    pub fn fold_event(&mut self, event: &Event, tick: u32) {
        match event {
            Event::RallyCall {
                ...
                self.fold_one(observer.raw(), wounded_kin.raw(), 1.0, tick);
            }
            _ => {}
        }
    }
