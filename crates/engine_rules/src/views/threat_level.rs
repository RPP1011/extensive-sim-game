...
/// @decay(rate = 0.98, per = tick) — anchor-pattern decay applied on read.
...
impl ThreatLevel {
    ...
    /// Current value for `b` as observed by `a`,
    /// decayed from its anchor tick. Returns `initial` when the pair isn't in a's top-8.
    ...
    /// Σ get(a, x, tick) over `a`'s top-8 slots.
