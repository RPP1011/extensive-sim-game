//! `target_chaser_runtime` build script. Mirrors
//! `particle_collision_runtime/build.rs` verbatim except for the
//! input fixture path (`target_chaser.sim`).
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/target_chaser_runtime");
    let sim_path = workspace_root.join("assets/sim/target_chaser.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse target_chaser.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve target_chaser.sim");
    let cg = dsl_compiler::cg::lower::lower_compilation_to_cg(&comp)
        .expect("lower target_chaser.sim to CG");
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit target_chaser CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[target_chaser emit-stats] {} kernels, schedule has {} stages",
    ...
    for (kernel_name, key) in artifacts.kernel_index.iter().zip(kernel_keys.iter()) {
        ...
        println!(
            "cargo:warning=[target_chaser emit-stats]   {kernel_name}: {bytes} B, {lines} lines, {bindings} bindings, {inline_consts} inline_consts",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by target_chaser_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/target_chaser.sim and rebuilding.\n\n",
    );
