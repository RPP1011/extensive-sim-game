//! Crowd-navigation Stage-0 demo. Drives the
//! [`crowd_navigation_runtime`] integrator through the same generic
//! harness shape as `sim_app` / `pp_app` / `pc_app`. Stage 0 just
//! integrates pos += vel × speed per tick; later stages copy the
//! per-walker mode-state-machine view, group memberships +
//! leader-driven goal updates, and utility-backend scoring rows
...
fn main() {
    ...
    println!(
        "cn_app: starting run — seed=0x{:016X} walkers={} ticks={}",
    ...
    println!(
        "cn_app: finished — final tick={} walkers={}",
        sim.tick(),
        sim.agent_count(),
    );
}
...
fn log_sample(sim: &mut CrowdNavigationState) {
    ...
    if positions.is_empty() {
        println!("  tick {:>4}: (no walkers)", tick);
        return;
    }
