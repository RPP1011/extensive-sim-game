//! `threats_with_decay_probe_runtime` build script. Mirrors
//! `threats_view_probe_runtime/build.rs` shape verbatim except for
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/threats_with_decay_probe_runtime");
    let sim_path = workspace_root.join("assets/sim/threats_with_decay_probe.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse threats_with_decay_probe.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve threats_with_decay_probe.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[threats_with_decay_probe lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit threats_with_decay_probe CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[threats_with_decay_probe emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[threats_with_decay_probe emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }

    for (name, body) in &artifacts.wgsl_files {
        fs::write(out_dir.join(name), body).unwrap_or_else(|e| panic!("write {}: {e}", name));
    }

    let mut generated = String::new();
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by threats_with_decay_probe_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/threats_with_decay_probe.sim and rebuilding.\n\n",
    );
