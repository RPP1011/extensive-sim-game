//! `spy_network_runtime` build script. Mirrors `duel_1v1_runtime`'s
//! build.rs verbatim except for the input fixture path AND the
//! `belief_state: true` opt-in (needed because the .sim's
//! `ApplyDisguise` rule writes `agents.set_disguise_*` SoA columns —
//! actually those are per-agent setters that don't require
//! belief_state, but mirroring tom_probe's build script for parity).
//!
//! Lowers `assets/sim/spy_network.sim` through the DSL compiler
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics — spy_network inherits duel_1v1's
//! known-deferred well_formed warnings (P6 + cycle: chronicle physics
//! writes agent.hp/alive/mana on the same ring the verb scoring
//! reads).
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/spy_network_runtime");
    let sim_path = workspace_root.join("assets/sim/spy_network.sim");

    println!("cargo:rerun-if-changed={}", sim_path.display());
    println!("cargo:rerun-if-changed=build.rs");

    let corpus_dir = workspace_root.join("assets/ability_test/spy_network");
    for name in ["Disguise.ability", "Slander.ability", "Strike.ability"] {
    ...
    let program = dsl_compiler::parse(&src).expect("parse spy_network.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve spy_network.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[spy_network lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit spy_network CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[spy_network emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[spy_network emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by spy_network_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/spy_network.sim and rebuilding.\n\n",
    );
