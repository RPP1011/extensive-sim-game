...
// The `ally_died_propagates_belief_row_via_merge` test that lived
// here was deleted in iter 21 — `belief_merge_propagation_probe_pin::
// ally_died_propagates_belief_row_to_every_receiver` provides the
// same end-to-end coverage (passing) using the fixture-internal-emit
// pattern, and its companion pair `room_known_pattern_probe_pin` /
// `belief_merge_ops_probe_pin` cover the single-key + max/min/replace
// variants. No need for an ignored duplicate.
