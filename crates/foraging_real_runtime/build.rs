//! `foraging_real_runtime` build script. Mirrors `duel_25v25_runtime`'s
//! build.rs shape verbatim except for the input fixture path.
//!
//! Lowers `assets/sim/foraging_real.sim` through the DSL compiler
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics — foraging_real inherits the
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/foraging_real_runtime");
    let sim_path = workspace_root.join("assets/sim/foraging_real.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse foraging_real.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve foraging_real.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[foraging_real lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit foraging_real CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[foraging_real emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[foraging_real emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by foraging_real_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/foraging_real.sim and rebuilding.\n\n",
    );
