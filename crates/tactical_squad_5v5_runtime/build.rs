//! `tactical_squad_5v5_runtime` build script. Mirrors `duel_1v1_runtime`'s
//! build.rs shape verbatim except for the input fixture path and the
//! list of tolerated lower diagnostics.
//!
//! Lowers `assets/sim/tactical_squad_5v5.sim` through the DSL compiler
//! pipeline (parse → resolve → CG lower → schedule → emit). Tolerates
//! the same well_formed warnings duel_1v1 surfaces: chronicle physics
//! writes hp/alive while mask + scoring read hp — a known cycle that
//! the schedule synthesizer handles via op-id ordering.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/tactical_squad_5v5_runtime");
    let sim_path = workspace_root.join("assets/sim/tactical_squad_5v5.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse tactical_squad_5v5.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve tactical_squad_5v5.sim");
    // Tolerate lower diagnostics — same shape as duel_1v1 (P6 + cycle
    // warnings around chronicle hp/alive writes vs mask/scoring reads).
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[tactical_squad_5v5 lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit tactical_squad_5v5 CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[tactical_squad_5v5 emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[tactical_squad_5v5 emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by tactical_squad_5v5_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/tactical_squad_5v5.sim and rebuilding.\n\n",
    );
