...
use crate::actions::{SpendPriority, StepDeltas, WorldEvent};
use crate::state::{AdventurerStatus, CampaignState, CAMPAIGN_TURN_SECS};
...
fn tick_trade_income(state: &mut CampaignState, dt_sec: f32) {
    ...
    let at_war_factions: Vec<usize> = state
        ...
        .filter(|f| {
            f.at_war_with.contains(&guild_faction_id)
                || matches!(
                    f.diplomatic_stance,
                    crate::state::DiplomaticStance::AtWar
                )
        })
        .map(|f| f.id)
        .collect();
    ...
    for region in &state.overworld.regions {
        ...
        let has_settlement = state.overworld.locations.iter().any(|loc| {
            loc.location_type == crate::state::LocationType::Settlement
...
fn tick_supply_chain(state: &mut CampaignState, dt_sec: f32) {
    ...
    let party_info: Vec<(u32, (f32, f32), f32, Vec<u32>)> = state
        ...
        .filter(|p| {
            matches!(
                p.status,
                crate::state::PartyStatus::Traveling
                    | crate::state::PartyStatus::OnMission
                    | crate::state::PartyStatus::Fighting
            )
        })
