    ...
    match &contract.task {
        ...
        ContractTask::TrainAdventurers { count, min_level } => {
            ...
            let qualified = state
                .adventurers
                .iter()
                .filter(|a| a.level >= *min_level && a.status != crate::state::AdventurerStatus::Dead)
                .count();
...
mod tests {
    ...
    fn make_test_state() -> CampaignState {
        ...
        state.phase = crate::state::CampaignPhase::Playing;
        state.tick = 1000;
        state
    }
    ...
    fn completed_contract_awards_rewards() {
        ...
        state.adventurers.push(crate::state::Adventurer {
            id: 10,
            name: "Test Hero".into(),
            ...
            traits: Vec::new(),
            status: crate::state::AdventurerStatus::Idle,
            loyalty: 70.0,
            stress: 0.0,
            fatigue: 0.0,
            injury: 0.0,
