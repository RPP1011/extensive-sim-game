...
fn complete_chain(state: &mut CampaignState, chain_idx: usize, events: &mut Vec<WorldEvent>) {
    ...
    for adv in &mut state.adventurers {
        if adv.status == crate::state::AdventurerStatus::Dead {
            continue;
        }
        *adv.history_tags
    ...
    let artifact = crate::state::Artifact {
        ...
        special_effect: match theme.as_str() {
            "vengeance" => crate::state::ArtifactEffect::CombatPowerBoost(0.08),
            "fortune" => crate::state::ArtifactEffect::XpMultiplier(1.15),
            "diplomacy" => crate::state::ArtifactEffect::FactionInfluence(0.1),
            "defense" => crate::state::ArtifactEffect::MoraleAura(5.0),
            "arcane" => crate::state::ArtifactEffect::HealingBoost(0.1),
            _ => crate::state::ArtifactEffect::None,
        },
