...
use crate::state::{CampaignState, ChronicleEntry, ChronicleType};
...
fn convert_event(state: &CampaignState, event: &WorldEvent) -> Option<ChronicleEntry> {
    ...
    match event {
        ...
        WorldEvent::QuestCompleted { quest_id, result } => {
            ...
            if threat <= 0.0 && *result != crate::state::QuestResult::Victory {
                return None;
            }
            let sig = 5.0 + threat / 20.0;
            let quest_desc = cq
        ...
        WorldEvent::BattleEnded { battle_id, result } => {
            ...
            match result {
                crate::state::BattleStatus::Victory => {
                ...
                crate::state::BattleStatus::Defeat => {
                    Some(ChronicleEntry {
                        tick,
