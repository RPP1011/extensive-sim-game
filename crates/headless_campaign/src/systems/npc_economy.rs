...
pub fn effective_level(adv: &crate::state::Adventurer) -> f32 {
...
fn equipment_value(adv: &crate::state::Adventurer) -> f32 {
...
pub fn combat_effective_level(adv: &crate::state::Adventurer) -> f32 {
...
fn initialize_npc_locations(state: &mut CampaignState) {
    ...
    let settlement_ids: Vec<usize> = state
        ...
        .locations
        .iter()
        .filter(|l| l.location_type == crate::state::LocationType::Settlement)
        .map(|l| l.id)
        .collect();
    ...
    for adv in &mut state.adventurers {
        ...
        adv.economic_intent = crate::state::EconomicIntent::Working;
        slot += 1;
    }
...
fn tags_to_production(adv: &crate::state::Adventurer) -> Vec<(usize, f32)> {
...
fn terrain_commodity_modifier(loc: &crate::state::Location, commodity: usize) -> f32 {
...
fn tick_production(state: &mut CampaignState) {
    ...
    for adv in &state.adventurers {
        ...
        if !matches!(adv.economic_intent, crate::state::EconomicIntent::Working) {
            continue;
        }
            ...
            if commodity == COMMODITY_EQUIPMENT || commodity == COMMODITY_MEDICINE {
                ...
                let terrain_mod = terrain_commodity_modifier(
                    ...
                    &crate::state::Location {
                        ...
                        local_prices: crate::state::BASE_PRICES,
                    },
                    commodity,
                );
...
fn tick_consumption(state: &mut CampaignState) {
    ...
    for adv in &mut state.adventurers {
        ...
        if is_starving {
            ...
            if adv.injury >= 95.0 {
                let roll = crate::state::lcg_f32(&mut state.rng);
...
fn tick_local_prices(state: &mut CampaignState) {
    ...
    for loc in &mut state.overworld.locations {
        ...
        for i in 0..8 {
            ...
            loc.local_prices[i] = crate::state::BASE_PRICES[i]
...
fn tick_information(state: &mut CampaignState) {
    ...
    for &loc_id in &settlement_ids {
        ...
        for _ in 0..exchanges {
            let idx_a = (crate::state::lcg_next(&mut state.rng) as usize) % resident_idxs.len();
            let idx_b = (crate::state::lcg_next(&mut state.rng) as usize) % resident_idxs.len();
...
fn tick_trade_decisions(state: &mut CampaignState, _events: &mut Vec<WorldEvent>) {
    use crate::state::{EconomicIntent, PartyPurpose};
    ...
    for (idx, adv) in state.adventurers.iter().enumerate() {
        ...
        let local_prices = state.overworld.locations.iter()
            ...
            .unwrap_or(crate::state::BASE_PRICES);
...
fn apply_monster_damage_to_settlements(state: &mut CampaignState, _events: &mut Vec<WorldEvent>) {
    ...
    for faction in &state.factions {
        if faction.at_war_with.contains(&guild_fid) || matches!(faction.diplomatic_stance, crate::state::DiplomaticStance::AtWar) {
        ...
        if defense_ratio >= 1.0 {
            ...
            if let Some(loc) = state.overworld.locations.iter_mut().find(|l| l.id == snap.loc_id) {
                ...
                for _ in 0..injury_count.min(resident_ids.len()) {
                    let idx = (crate::state::lcg_next(&mut state.rng) as usize) % resident_ids.len();
            ...
            if let Some(loc) = state.overworld.locations.iter_mut().find(|l| l.id == snap.loc_id) {
                ...
                for _ in 0..casualty_count.min(resident_ids.len()) {
                    ...
                    if let Some(adv) = state.adventurers.iter_mut().find(|a| a.id == adv_id) {
                        ...
                        adv.stress = 100.0;
                        if adv.injury > 80.0 {
                            let death_roll = crate::state::lcg_f32(&mut state.rng);
...
fn apply_npc_income(state: &mut CampaignState, _events: &mut Vec<WorldEvent>) {
    let cfg = state.config.npc_economy.clone();
    ...
    for adv in &mut state.adventurers {
        ...
        let roll = crate::state::lcg_f32(rng);
...
fn apply_travel_danger(state: &mut CampaignState, events: &mut Vec<WorldEvent>) {
    ...
    let traveling_parties: Vec<(u32, Vec<u32>, (f32, f32))> = state
        .parties
        .iter()
        .filter(|p| p.autonomous && matches!(p.status, crate::state::PartyStatus::Traveling))
        .map(|p| (p.id, p.member_ids.clone(), p.position))
        .collect();

    for (party_id, member_ids, _position) in &traveling_parties {
        ...
        if roll < injury_chance && !member_ids.is_empty() {
            let target_idx = (crate::state::lcg_f32(&mut state.rng) * member_ids.len() as f32) as usize;
...
fn run_npc_decisions(state: &mut CampaignState, events: &mut Vec<WorldEvent>) {
    use crate::state::{EconomicIntent, PartyPurpose, GoalType};
        ...
        if founding_utility > best * 1.1 {
            ...
            let base_pos = current_loc
                ...
                .map(|_| {
                    ...
                    let offset_x = (crate::state::lcg_f32(&mut state.rng) - 0.5) * 40.0;
                    let offset_y = (crate::state::lcg_f32(&mut state.rng) - 0.5) * 40.0;
...
fn estimate_income(
    ...
    lvl_mult: f32,
    demand: &[f32; 8],
    cfg: &crate::config::NpcEconomyConfig,
) -> f32 {
    let exp = cfg.demand_income_exponent;
    let base_rate = cfg.service_income_per_stat_point;
...
fn run_party_formation(state: &mut CampaignState, events: &mut Vec<WorldEvent>) {
    ...
    let mut new_parties: Vec<crate::state::Party> = Vec::new();
    ...
    for (_loc_id, seekers) in &seekers_by_loc {
        ...
        if adventure_seekers.len() >= 2 {
            ...
            if max_combat > 5.0 {
                ...
                new_parties.push(crate::state::Party {
                    id: next_party_id,
...
fn manage_adventuring_parties(state: &mut CampaignState, events: &mut Vec<WorldEvent>) {
    use crate::state::{EconomicIntent, AutonomousPartyType, PartyPurpose};
        ...
        if let Some(adv) = state.adventurers.iter_mut().find(|a| a.id == *adv_id) {
            ...
            if adv.home_location_id.is_none() {
                if let Some(loc) = state.overworld.locations.iter().find(|l| {
                    l.location_type == crate::state::LocationType::Settlement
                }) {
            ...
            for mid in &member_ids {
                if let Some(adv) = state.adventurers.iter_mut().find(|a| a.id == *mid) {
                    ...
                    if adv.home_location_id.is_none() {
                        if let Some(loc) = state.overworld.locations.iter().find(|l| {
                            l.location_type
                                == crate::state::LocationType::Settlement
                        }) {
                            adv.home_location_id = Some(loc.id);
                        }
                    }
    ...
    for party in &state.parties {
        ...
        let arrived = party.destination.is_none()
            || party.status == crate::state::PartyStatus::Idle;
        if arrived {
    ...
    for (_party_id, pos, member_ids) in &new_settlements {
        ...
        state.overworld.locations.push(crate::state::Location {
            id: new_loc_id,
            name: settlement_name.clone(),
            position: *pos,
            location_type: crate::state::LocationType::Settlement,
            ...
            stockpile: {
                ...
                crate::state::CommodityStockpile {
...
fn manage_patronage(state: &mut CampaignState, _events: &mut Vec<WorldEvent>) {
    use crate::state::PatronageContract;
...
fn check_bandit_transition(state: &mut CampaignState, events: &mut Vec<WorldEvent>) {
    ...
    for &idx in &new_bandits {
        ...
        adv.economic_intent = crate::state::EconomicIntent::Idle;
        adv.loyalty = 0.0;
...
mod tests {
    ...
    fn test_adventurer(id: u32, level: u32, gold: f32) -> crate::state::Adventurer {
        crate::state::Adventurer {
            id,
            name: format!("Test-{id}"),
            archetype: "knight".to_string(),
            level,
            xp: 0,
            stats: crate::state::AdventurerStats {
    ...
    fn test_income_scales_with_level() {
        let cfg = crate::config::NpcEconomyConfig::default();
        let demand = [0.5; 8];
