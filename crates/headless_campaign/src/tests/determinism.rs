...
use crate::actions::CampaignAction;
use crate::state::{CampaignPhase, CampaignState};
use crate::step::step_campaign;
...
fn test_quest_lifecycle() {
    ...
    assert!(matches!(result.action_result, Some(crate::ActionResult::Success(_))));
    assert_eq!(state.active_quests.len(), 1);
    assert_eq!(state.active_quests[0].status, crate::state::ActiveQuestStatus::Preparing);
    ...
    assert!(
        !matches!(
            ...
            Some(crate::state::ActiveQuestStatus::Preparing)
        ),
