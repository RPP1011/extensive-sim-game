...
/// Compute flat power bonus from active unlocks.
pub fn unlock_power_bonus(unlocks: &[UnlockInstance]) -> f32 {
    let mut bonus = 0.0;
    for unlock in unlocks {
        if !unlock.active { continue; }
        match unlock.category {
            UnlockCategory::PassiveBuff => bonus += unlock.properties.magnitude * 20.0,
            UnlockCategory::ActiveAbility => bonus += unlock.properties.magnitude * 10.0,
            UnlockCategory::Information => bonus += unlock.properties.magnitude * 5.0,
