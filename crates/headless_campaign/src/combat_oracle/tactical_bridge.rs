...
use crate::state::{Adventurer, CampaignState, UnlockCategory, UnlockInstance};
...
fn adventurer_to_unit(
    ...
    for prog in &state.pending_progression {
        if prog.adventurer_id == Some(adventurer.id)
            && prog.kind == crate::state::ProgressionKind::Ability
        {
...
fn try_load_hero_template(archetype: &str) -> Option<tactical_sim::effects::HeroToml> {
    ...
    for path in &paths {
        if let Ok(content) = std::fs::read_to_string(path) {
            ...
            if let Ok(mut toml) = toml::from_str::<tactical_sim::effects::HeroToml>(&content) {
                if let Some(dsl_str) = dsl_content.as_deref() {
                    if let Ok((abilities, passives)) = tactical_sim::effects::dsl::parse_abilities(dsl_str) {
...
fn build_enemies(
    ...
    let mut enemies =
        default_enemy_wave(enemy_count, seed, &spawn_positions);
...
fn hero_toml_to_unit(toml: &tactical_sim::effects::HeroToml, id: u32, team: Team, position: SimVec2) -> UnitState {
    let atk = toml.attack.clone().unwrap_or_default();
    let abilities = toml.abilities.iter().map(|d| AbilitySlot::new(d.clone())).collect();
    let passives = toml.passives.iter().map(|d| tactical_sim::effects::PassiveSlot::new(d.clone())).collect();
    UnitState {
        id, team, hp: toml.stats.hp, max_hp: toml.stats.hp, position,
        move_speed_per_sec: toml.stats.move_speed,
        attack_damage: atk.damage, attack_range: atk.range,
        attack_cooldown_ms: atk.cooldown, attack_cast_time_ms: atk.cast_time,
        cooldown_remaining_ms: 0,
        ...
        control_cast_time_ms: 0, control_cooldown_remaining_ms: 0, control_remaining_ms: 0,
        casting: None, abilities, passives,
        status_effects: Vec::new(), shield_hp: 0,
        resistance_tags: toml.stats.tags.clone(),
        state_history: VecDeque::new(), channeling: None,
        resource: toml.stats.resource, max_resource: toml.stats.max_resource,
        resource_regen_per_sec: toml.stats.resource_regen_per_sec,
        owner_id: None, directed: false,
        armor: toml.stats.armor, magic_resist: toml.stats.magic_resist,
        cover_bonus: 0.0, elevation: 0.0, total_healing_done: 0, total_damage_done: 0,
    }
}

fn default_enemy_wave(count: usize, seed: u64, positions: &[SimVec2]) -> Vec<UnitState> {
    let mut rng = seed;
    (0..count).map(|i| {
        rng = rng.wrapping_mul(6364136223846793005).wrapping_add(1);
        let v = (rng >> 32) as u32 % 3;
        let (hp, atk, spd, range) = match v {
            0 => (80, 12, 2.5_f32, 2.0_f32),
            1 => (50, 18, 3.0, 2.0),
            _ => (60, 15, 2.0, 6.0),
        };
        let pos = positions[i % positions.len()];
        UnitState {
            id: 1000 + i as u32, team: Team::Enemy,
            hp, max_hp: hp, position: pos,
            move_speed_per_sec: spd, attack_damage: atk, attack_range: range,
            attack_cooldown_ms: 1000, attack_cast_time_ms: 200, cooldown_remaining_ms: 0,
            ability_damage: 0, ability_range: 0.0, ability_cooldown_ms: 0,
            ability_cast_time_ms: 0, ability_cooldown_remaining_ms: 0,
            heal_amount: 0, heal_range: 0.0, heal_cooldown_ms: 0,
            heal_cast_time_ms: 0, heal_cooldown_remaining_ms: 0,
            control_range: 0.0, control_duration_ms: 0, control_cooldown_ms: 0,
            control_cast_time_ms: 0, control_cooldown_remaining_ms: 0, control_remaining_ms: 0,
            casting: None, abilities: Vec::new(), passives: Vec::new(),
            status_effects: Vec::new(), shield_hp: 0,
            resistance_tags: Tags::default(), state_history: VecDeque::new(),
            channeling: None, resource: 0, max_resource: 0, resource_regen_per_sec: 0.0,
            owner_id: None, directed: false, armor: 0.0, magic_resist: 0.0,
            cover_bonus: 0.0, elevation: 0.0, total_healing_done: 0, total_damage_done: 0,
        }
    }).collect()
}
...
mod tests {
    ...
    fn test_adventurer(id: u32, archetype: &str, level: u32, attack: f32, defense: f32) -> Adventurer {
        Adventurer {
            ...
            deeds: Vec::new(),
            hobbies: Vec::new(),
            disease_status: crate::state::DiseaseStatus::Healthy,

            mood_state: crate::state::MoodState::default(),
