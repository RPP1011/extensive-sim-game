//! `duel_25v25_runtime` build script. Mirrors `duel_1v1_runtime`'s
//! build.rs shape verbatim except for the input fixture path.
//!
//! Lowers `assets/sim/duel_25v25.sim` through the DSL compiler
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics — duel_25v25 inherits the duel_1v1
//! P6 + cycle warnings (chronicle physics writes agent.hp/alive on
//! the same ring scoring/mask might read; the well_formed checker
//! doesn't yet model @phase(post) authored mutation as legitimate).
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/duel_25v25_runtime");
    let sim_path = workspace_root.join("assets/sim/duel_25v25.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse duel_25v25.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve duel_25v25.sim");
    ...
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        ...
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[duel_25v25 lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit duel_25v25 CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[duel_25v25 emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[duel_25v25 emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by duel_25v25_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/duel_25v25.sim and rebuilding.\n\n",
    );
