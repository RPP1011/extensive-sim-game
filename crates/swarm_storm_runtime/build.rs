//! `swarm_storm_runtime` build script. Mirrors the other fixture
//! runtimes' build.rs shape; only the input fixture path differs.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/swarm_storm_runtime");
    let sim_path = workspace_root.join("assets/sim/swarm_event_storm.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse swarm_event_storm.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve swarm_event_storm.sim");
    let cg = dsl_compiler::cg::lower::lower_compilation_to_cg(&comp)
        .expect("lower swarm_event_storm.sim to CG");
    ...
    println!(
        "cargo:warning=[swarm_event_storm emit-stats] {} kernels, schedule has {} stages",
    ...
    for (kernel_name, key) in artifacts.kernel_index.iter().zip(kernel_keys.iter()) {
        ...
        println!(
            "cargo:warning=[swarm_event_storm emit-stats]   {kernel_name}: {bytes} B, {lines} lines, {bindings} bindings, {inline_consts} inline_consts",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by swarm_storm_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/swarm_event_storm.sim and rebuilding.\n\n",
    );
