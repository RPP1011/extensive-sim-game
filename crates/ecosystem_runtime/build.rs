//! `ecosystem_runtime` build script. Mirrors `predator_prey_runtime`'s
//! build.rs verbatim except for the input fixture path. Tracks
//! `assets/sim/ecosystem_cascade.sim`.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/ecosystem_runtime");
    let sim_path = workspace_root.join("assets/sim/ecosystem_cascade.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse ecosystem_cascade.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve ecosystem_cascade.sim");
    let cg = dsl_compiler::cg::lower::lower_compilation_to_cg(&comp)
        .expect("lower ecosystem_cascade.sim to CG");
    ...
    .expect("emit ecosystem_cascade CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[ecosystem emit-stats] {} kernels, schedule has {} stages",
    ...
    for (kernel_name, key) in artifacts.kernel_index.iter().zip(kernel_keys.iter()) {
        ...
        println!(
            "cargo:warning=[ecosystem emit-stats]   {kernel_name}: {bytes} B, {lines} lines, {bindings} bindings, {inline_consts} inline_consts",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by ecosystem_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/ecosystem_cascade.sim and rebuilding.\n\n",
    );
