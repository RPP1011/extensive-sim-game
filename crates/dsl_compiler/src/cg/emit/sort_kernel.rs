...
// Scatter kernels are dispatched as @workgroup_size(1) for full
// determinism: a single thread iterates events sequentially, so the
// atomicAdd intra-bucket position is monotonic in input order. For
// fixture event volumes up to ~10k/tick this is microseconds-scale
// overhead — acceptable for the determinism contract.
...
"#);
    ...
    // -- Scatter kernel — single-threaded sequential for deterministic intra-bucket order.
...
fn radix_stage_b_scatter(@builtin(local_invocation_id) lid: vec3<u32>) {{
    let count = atomicLoad(&event_tail);
    for (var tid = 0u; tid < count; tid = tid + 1u) {{
        let tgt_id = atomicLoad(&event_ring_in[tid * {stride}u + cfg.target_word_offset]);
        let bucket = select(tgt_id, cfg.agent_cap, tgt_id >= cfg.agent_cap);
