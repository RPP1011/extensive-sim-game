...
use crate::cg::lower::driver::DebugDepth;
...
pub fn synthesize_dispatch(kernel_index: &[String]) -> String {
    synthesize_dispatch_with_debug(kernel_index, DebugDepth::Off, &[])
}

/// Same as [`synthesize_dispatch`] but emits compiler debug-mode
/// instrumentation per the supplied [`DebugDepth`].
///
/// `dsl_source_map` is consulted only at [`DebugDepth::DslMapped`] to
/// populate the emitted `KERNEL_DSL_SOURCE_MAP` table; entries are
/// `(kernel_name, sim_file_path, line_start, line_end)` tuples keyed
/// by `kernel_name`. The emitter walks `kernel_index` in order and
/// looks up each entry by name; kernels without a matching entry
/// surface as `("", 0, 0)`, signalling "compiler couldn't trace this
/// kernel back to a single .sim span".
///
/// 2026-05-09 Phase 1: D1+ adds a `DebugTimings` struct + per-kernel
/// `record_<name>_timing` helpers (alongside the existing
/// `dispatch_<name>` plain-dispatch helpers — strict superset of the
/// D0 surface). D2+ adds a `MemDelta` struct + `MemTrafficTable`
/// helper so runtimes can attribute host↔GPU bytes per kernel
/// without re-deriving them. D4 adds the `KERNEL_DSL_SOURCE_MAP`
/// static.
pub fn synthesize_dispatch_with_debug(
    kernel_index: &[String],
    debug: DebugDepth,
    dsl_source_map: &[(String, String, u32, u32)],
) -> String {
    let mut out = String::new();
    out.push_str("// GENERATED by dsl_compiler — do not edit by hand.\n");
    out.push_str("// Regenerated by each runtime crate's build.rs on cargo check / build.\n");
    out.push_str("//!\n");
    out.push_str("//! Per-kernel cache + dispatch helpers. Each per-fixture runtime\n");
    out.push_str("//! crate uses these to lazy-init kernels and record dispatches\n");
    out.push_str("//! against its own per-field buffers.\n");
    out.push_str("//!\n");
    out.push_str("//! Phase 7 boids GPU pipeline (2026-05-02): rewrote against\n");
    out.push_str("//! `engine::gpu::Kernel`. The previous `dispatch_by_id` generic\n");
    out.push_str("//! function (which built the cfg uniform + called bind() against a\n");
    out.push_str("//! `BindingSources` aggregate) is gone — `Cfg` is `#[repr(C)]` POD\n");
    out.push_str("//! the runtime constructs directly, and `Bindings` is wired inline\n");
    out.push_str("//! at each per-kernel dispatch helper's call site.\n");
    if debug != DebugDepth::Off {
        writeln!(
            out,
            "//!\n//! 2026-05-09 compiler debug mode: this build was lowered with\n\
             //! `LowerOpts::debug = DebugDepth::{:?}` (D{}). The dispatcher\n\
             //! exposes additional helpers gated to the level requested.",
            debug,
            debug as u8,
        )
        .expect("write to String");
    ...
    out.push_str("/// `<engine::gpu::Kernel>::new(device)`.\n");
    ...
    // Per-kernel dispatch helpers. Each one:
    //   1. Lazy-inits the kernel via the cache.
    //   2. Calls record() with the caller-supplied bindings + agent_cap.
    //
    // Bindings construction is the runtime's responsibility — boids_runtime
    // (and any future per-fixture runtime) builds the `<Kernel>Bindings`
    // struct inline at the call site with refs to its own per-field
    // wgpu::Buffers. No central BindingSources aggregate.
    for name in kernel_index {
        let pascal = snake_to_pascal(name);
        let field = name.to_lowercase();
        writeln!(
            out,
            "/// Lazy-init + record [`crate::{name}::{pascal}Kernel`]\n\
             /// against `bindings` into `encoder`. The kernel is\n\
             /// constructed on first call via\n\
             /// `<engine::gpu::Kernel>::new(device)` and cached for\n\
             /// the rest of the run."
        )
        .expect("write to String");
        writeln!(
            out,
            "pub fn dispatch_{field}(\n    \
                 cache: &mut KernelCache,\n    \
                 bindings: &crate::{name}::{pascal}Bindings<'_>,\n    \
                 device: &wgpu::Device,\n    \
        ...
        writeln!(
            out,
            "    let kernel = cache.{field}.get_or_insert_with(|| \
                 <crate::{name}::{pascal}Kernel as engine::gpu::Kernel>::new(device));"
        )
        .expect("write to String");
    ...
    if debug.emits_timestamps() {
        emit_debug_timing_surface(&mut out, kernel_index, debug);
    }
    if debug.emits_memory_traffic() {
        emit_debug_memory_surface(&mut out, kernel_index);
    }
    if debug.emits_source_map() {
        emit_debug_source_map(&mut out, kernel_index, dsl_source_map);
    }

    out
}

/// Emit the D1+ per-kernel timestamp helper surface. The runtime
/// constructs `DebugTimings::new(&gpu)` once at startup (None when
/// the adapter doesn't expose `wgpu::Features::TIMESTAMP_QUERY`),
/// then per tick calls `record_<name>_timing(&mut timings, ...)` in
/// place of `dispatch_<name>(...)`. After `queue.submit + device.poll`,
/// `timings.read_kernel_timings_with(device)` returns one `(name, ns)`
/// per kernel for the most recently completed tick.
fn emit_debug_timing_surface(out: &mut String, kernel_index: &[String], debug: DebugDepth) {
    let n_kernels = kernel_index.len() as u32;
    // We bracket each kernel with two timestamps (before / after); the
    // total slot count is `2 * n_kernels` for per-kernel granularity
    // (D3+) or `2 * n_stages` for per-stage granularity (D1/D2). For
    // simplicity Phase 1 uses per-kernel slots at every level >= D1
    // — a stage is, in the current schedule, frequently a single
    // kernel anyway, and the runtime can collapse contiguous kernels
    // into a "stage" view downstream.
    let slot_count = n_kernels.saturating_mul(2);
    out.push_str(
        "// ----------------------------------------------------------------\n\
         // Compiler debug mode (D1+): per-kernel GPU timestamps.\n\
         // ----------------------------------------------------------------\n\n",
    );
    out.push_str("/// Per-kernel GPU timing. Returned by\n");
    out.push_str("/// [`DebugTimings::read_kernel_timings_with`] — one entry per\n");
    out.push_str("/// `record_<name>_timing` call this tick (in dispatch order).\n");
    out.push_str("/// `wall_ns` is the GPU-side wall time between this kernel's\n");
    out.push_str("/// pre-write_timestamp and post-write_timestamp.\n");
    out.push_str("#[derive(Debug, Clone, Default)]\n");
    out.push_str("pub struct KernelTiming {\n");
    out.push_str("    pub kernel: String,\n");
    out.push_str("    pub wall_ns: u64,\n");
    out.push_str("}\n\n");

    // KERNEL_NAMES static table — one entry per kernel, in dispatch
    // order, used by the readback to label the resolved nanos.
    out.push_str("/// Stable ordering of every emitted kernel. The N'th entry maps\n");
    out.push_str("/// 1:1 to timestamp slots `2*N` (pre) and `2*N+1` (post).\n");
    writeln!(out, "pub const KERNEL_NAMES: &[&str] = &[").expect("write to String");
    for name in kernel_index {
        writeln!(out, "    {:?},", name).expect("write to String");
    }
    out.push_str("];\n\n");

    writeln!(out, "/// Total timestamp slots required by `DebugTimings` —")
        .expect("write to String");
    writeln!(out, "/// `2 * KERNEL_NAMES.len()` (pre / post per kernel).")
        .expect("write to String");
    writeln!(out, "pub const TIMING_SLOT_COUNT: u32 = {};", slot_count)
        .expect("write to String");
    writeln!(
        out,
        "/// True when this dispatch.rs was emitted with per-kernel timestamp\n\
         /// granularity (D3+). False for D1/D2; the runtime can use this to\n\
         /// decide whether to collapse adjacent slots into a stage rollup.",
    )
    .expect("write to String");
    writeln!(
        out,
        "pub const DEBUG_DEPTH_PER_KERNEL: bool = {};",
        debug.per_kernel_granularity()
    )
    .expect("write to String");
    writeln!(out, "pub const DEBUG_DEPTH_LEVEL: u8 = {};", debug as u8)
        .expect("write to String");
    out.push('\n');

    // The DebugTimings struct itself.
    out.push_str(
        "/// Wraps a `wgpu::QuerySet` + resolve / readback buffers used\n\
         /// to capture per-kernel GPU wall time. Construct via\n\
         /// [`DebugTimings::new`] once at runtime startup; then bracket\n\
         /// each emitted kernel's record call via\n\
         /// `record_<name>_timing(...)`. After `queue.submit + device.poll`,\n\
         /// drain the latest tick's timings via\n\
         /// [`DebugTimings::read_kernel_timings_with`].\n\
         ///\n\
         /// `DebugTimings::new` returns `None` when the adapter doesn't\n\
         /// expose `wgpu::Features::TIMESTAMP_QUERY` — runtimes should\n\
         /// fall back to the non-instrumented dispatch path (P10).\n",
    );
    out.push_str("pub struct DebugTimings {\n");
    out.push_str("    pub query_set: wgpu::QuerySet,\n");
    out.push_str("    pub resolve_buf: wgpu::Buffer,\n");
    out.push_str("    pub readback_buf: wgpu::Buffer,\n");
    out.push_str("    pub period_ns: f32,\n");
    out.push_str("    pub slot_cursor: std::cell::Cell<u32>,\n");
    out.push_str("    pub has_pending: std::cell::Cell<bool>,\n");
    out.push_str("    /// Names of the kernels that were record_<name>_timing'd this tick,\n");
    out.push_str("    /// in dispatch order. Not every emitted kernel is necessarily\n");
    out.push_str("    /// dispatched per tick (e.g. plumbing/snapshot kernels run only at\n");
    out.push_str("    /// specific cadences); this records what the runtime actually\n");
    out.push_str("    /// bracketed so the per-kernel timings line up with the call site.\n");
    out.push_str("    pub recorded_kernels: std::cell::RefCell<Vec<&'static str>>,\n");
    out.push_str("}\n\n");

    out.push_str("impl DebugTimings {\n");
    out.push_str("    /// Construct timing instrumentation for the supplied GPU\n");
    out.push_str("    /// context. Returns `None` when `TIMESTAMP_QUERY` is\n");
    out.push_str("    /// unavailable; the caller should treat that as \"timings\n");
    out.push_str("    /// disabled, dispatch normally\" rather than a hard error.\n");
    out.push_str("    pub fn new(gpu: &engine::GpuContext) -> Option<Self> {\n");
    out.push_str("        if !gpu.supports_timestamp_query() {\n");
    out.push_str("            return None;\n");
    out.push_str("        }\n");
    out.push_str("        let query_set = gpu.device.create_query_set(&wgpu::QuerySetDescriptor {\n");
    out.push_str("            label: Some(\"compiler_debug::timings\"),\n");
    out.push_str("            ty: wgpu::QueryType::Timestamp,\n");
    out.push_str("            count: TIMING_SLOT_COUNT,\n");
    out.push_str("        });\n");
    out.push_str("        let resolve_buf = gpu.device.create_buffer(&wgpu::BufferDescriptor {\n");
    out.push_str("            label: Some(\"compiler_debug::timing_resolve\"),\n");
    out.push_str("            size: (TIMING_SLOT_COUNT as u64) * 8,\n");
    out.push_str("            usage: wgpu::BufferUsages::COPY_SRC | wgpu::BufferUsages::QUERY_RESOLVE,\n");
    out.push_str("            mapped_at_creation: false,\n");
    out.push_str("        });\n");
    out.push_str("        let readback_buf = gpu.device.create_buffer(&wgpu::BufferDescriptor {\n");
    out.push_str("            label: Some(\"compiler_debug::timing_readback\"),\n");
    out.push_str("            size: (TIMING_SLOT_COUNT as u64) * 8,\n");
    out.push_str("            usage: wgpu::BufferUsages::COPY_DST | wgpu::BufferUsages::MAP_READ,\n");
    out.push_str("            mapped_at_creation: false,\n");
    out.push_str("        });\n");
    out.push_str("        Some(Self {\n");
    out.push_str("            query_set,\n");
    out.push_str("            resolve_buf,\n");
    out.push_str("            readback_buf,\n");
    out.push_str("            period_ns: gpu.queue.get_timestamp_period(),\n");
    out.push_str("            slot_cursor: std::cell::Cell::new(0),\n");
    out.push_str("            has_pending: std::cell::Cell::new(false),\n");
    out.push_str("            recorded_kernels: std::cell::RefCell::new(Vec::new()),\n");
    out.push_str("        })\n");
    out.push_str("    }\n\n");

    out.push_str("    /// Reset the slot cursor to zero — call once at the start of\n");
    out.push_str("    /// each tick before the first `record_<name>_timing` call.\n");
    out.push_str("    pub fn begin_tick(&self) {\n");
    out.push_str("        self.slot_cursor.set(0);\n");
    out.push_str("        self.recorded_kernels.borrow_mut().clear();\n");
    out.push_str("    }\n\n");

    out.push_str("    /// Resolve + copy the per-tick timestamp buffer into the\n");
    out.push_str("    /// readback buffer. Call once at end-of-tick after every\n");
    out.push_str("    /// `record_<name>_timing` has run, BEFORE `queue.submit`.\n");
    out.push_str("    pub fn finalise_tick(&self, encoder: &mut wgpu::CommandEncoder) {\n");
    out.push_str("        let used = self.slot_cursor.get();\n");
    out.push_str("        if used == 0 {\n");
    out.push_str("            return;\n");
    out.push_str("        }\n");
    out.push_str("        encoder.resolve_query_set(&self.query_set, 0..used, &self.resolve_buf, 0);\n");
    out.push_str("        encoder.copy_buffer_to_buffer(\n");
    out.push_str("            &self.resolve_buf,\n");
    out.push_str("            0,\n");
    out.push_str("            &self.readback_buf,\n");
    out.push_str("            0,\n");
    out.push_str("            (used as u64) * 8,\n");
    out.push_str("        );\n");
    out.push_str("        self.has_pending.set(true);\n");
    out.push_str("    }\n\n");

    out.push_str("    /// Map the readback buffer + decode timestamp pairs into\n");
    out.push_str("    /// `KernelTiming` records. The runtime's typical sequence\n");
    out.push_str("    /// is: `finalise_tick(encoder)` → `queue.submit` →\n");
    out.push_str("    /// `device.poll(Wait)` → `read_kernel_timings()`. This\n");
    out.push_str("    /// method drives its own internal `device.poll(Wait)` so\n");
    out.push_str("    /// the `map_async` callback fires before we attempt the\n");
    out.push_str("    /// `get_mapped_range`; without that extra poll wgpu\n");
    out.push_str("    /// raises a `Buffer is not mapped` validation error.\n");
    out.push_str("    pub fn read_kernel_timings_with(&self, device: &wgpu::Device) -> Vec<KernelTiming> {\n");
    out.push_str("        if !self.has_pending.get() {\n");
    out.push_str("            return Vec::new();\n");
    out.push_str("        }\n");
    out.push_str("        let used = self.slot_cursor.get() as usize;\n");
    out.push_str("        if used == 0 {\n");
    out.push_str("            return Vec::new();\n");
    out.push_str("        }\n");
    out.push_str("        let slice = self.readback_buf.slice(..((used as u64) * 8));\n");
    out.push_str("        slice.map_async(wgpu::MapMode::Read, |r| {\n");
    out.push_str("            let _ = r;\n");
    out.push_str("        });\n");
    out.push_str("        // Drive a poll(Wait) so the map_async callback completes\n");
    out.push_str("        // before we touch the slice; the prior tick's GPU work\n");
    out.push_str("        // is already flushed by the caller's poll, but the\n");
    out.push_str("        // map_async callback runs on the next poll cycle.\n");
    out.push_str("        let _ = device.poll(wgpu::PollType::Wait);\n");
    out.push_str("        let raw: Vec<u64> = {\n");
    out.push_str("            let view = slice.get_mapped_range();\n");
    out.push_str("            let words: &[u64] = bytemuck::cast_slice(&view);\n");
    out.push_str("            words.to_vec()\n");
    out.push_str("        };\n");
    out.push_str("        self.readback_buf.unmap();\n");
    out.push_str("        self.has_pending.set(false);\n");
    out.push_str("        let period = self.period_ns as f64;\n");
    out.push_str("        let recorded = self.recorded_kernels.borrow();\n");
    out.push_str("        let mut out: Vec<KernelTiming> = Vec::with_capacity(used / 2);\n");
    out.push_str("        let pairs = used / 2;\n");
    out.push_str("        for i in 0..pairs {\n");
    out.push_str("            let lo = raw[i * 2];\n");
    out.push_str("            let hi = raw[i * 2 + 1];\n");
    out.push_str("            let ns = ((hi.saturating_sub(lo)) as f64 * period) as u64;\n");
    out.push_str("            let name = recorded\n");
    out.push_str("                .get(i)\n");
    out.push_str("                .copied()\n");
    out.push_str("                .unwrap_or(\"\")\n");
    out.push_str("                .to_string();\n");
    out.push_str("            out.push(KernelTiming { kernel: name, wall_ns: ns });\n");
    out.push_str("        }\n");
    out.push_str("        out\n");
    out.push_str("    }\n");
    out.push_str("}\n\n");

    // Per-kernel record_<name>_timing helpers. Each one writes its
    // kernel name onto `timings.recorded_kernels` so the readback can
    // attribute slot N's nanos to the correct kernel even when not
    // every emitted kernel runs every tick.
    for name in kernel_index {
        let pascal = snake_to_pascal(name);
        let field = name.to_lowercase();
        writeln!(
            out,
            "/// D1+ instrumented dispatch helper for `{name}`. Same as\n\
             /// [`dispatch_{field}`] but brackets the recorded dispatch in\n\
             /// `encoder.write_timestamp` calls bracketed by the current\n\
             /// `timings.slot_cursor` (which advances by 2 slots per call).",
        )
        .expect("write to String");
        writeln!(
            out,
            "pub fn record_{field}_timing(\n    \
                 cache: &mut KernelCache,\n    \
        ...
        writeln!(
            out,
            "    let pre = timings.slot_cursor.get();\n    \
                 encoder.write_timestamp(&timings.query_set, pre);"
        )
        .expect("write to String");
        writeln!(
            out,
        ...
        .expect("write to String");
        out.push_str("    kernel.record(device, encoder, bindings, agent_cap);\n");
        out.push_str("    encoder.write_timestamp(&timings.query_set, pre + 1);\n");
        out.push_str("    timings.slot_cursor.set(pre + 2);\n");
        writeln!(out, "    timings.recorded_kernels.borrow_mut().push({:?});", name)
            .expect("write to String");
        out.push_str("}\n\n");
    }
}

/// Emit the D2+ per-kernel memory-traffic accounting helpers. Today
/// every kernel reports the same shape (`MemDelta` of `host_to_gpu`
/// + `gpu_to_host` bytes); the per-fixture runtime accumulates by
/// calling `MemDelta::add_host_write(buf_len)` etc. immediately
/// before / after each `queue.write_buffer` / `queue.submit` call.
fn emit_debug_memory_surface(out: &mut String, kernel_index: &[String]) {
    out.push_str(
        "// ----------------------------------------------------------------\n\
         // Compiler debug mode (D2+): per-kernel host↔GPU memory traffic.\n\
         // ----------------------------------------------------------------\n\n",
    );
    out.push_str("/// Per-kernel memory-traffic accumulator. Captures host→GPU\n");
    out.push_str("/// (e.g. `queue.write_buffer`) and GPU→host (e.g. staging\n");
    out.push_str("/// readback) byte counts for one tick.\n");
    out.push_str("#[derive(Debug, Default, Clone, Copy)]\n");
    out.push_str("pub struct MemDelta {\n");
    out.push_str("    pub host_to_gpu_bytes: u64,\n");
    out.push_str("    pub gpu_to_host_bytes: u64,\n");
    out.push_str("}\n\n");

    out.push_str("impl MemDelta {\n");
    out.push_str("    pub fn add_host_write(&mut self, n: u64) { self.host_to_gpu_bytes += n; }\n");
    out.push_str("    pub fn add_host_read(&mut self, n: u64) { self.gpu_to_host_bytes += n; }\n");
    out.push_str("    pub fn reset(&mut self) {\n");
    out.push_str("        self.host_to_gpu_bytes = 0;\n");
    out.push_str("        self.gpu_to_host_bytes = 0;\n");
    out.push_str("    }\n");
    out.push_str("}\n\n");

    out.push_str("/// One `MemDelta` per emitted kernel, indexed in `KERNEL_NAMES`\n");
    out.push_str("/// order. Per-fixture runtimes call `traffic[idx]` to attribute\n");
    out.push_str("/// each per-tick `queue.write_buffer` / staging-readback byte\n");
    out.push_str("/// count to the kernel that consumes the buffer.\n");
    out.push_str("#[derive(Debug, Default, Clone)]\n");
    out.push_str("pub struct MemTrafficTable {\n");
    out.push_str("    pub per_kernel: Vec<MemDelta>,\n");
    out.push_str("}\n\n");

    out.push_str("impl MemTrafficTable {\n");
    writeln!(
        out,
        "    /// Construct an empty table sized for `KERNEL_NAMES` ({} kernels).",
        kernel_index.len(),
    )
    .expect("write to String");
    out.push_str("    pub fn new() -> Self {\n");
    out.push_str("        Self { per_kernel: vec![MemDelta::default(); KERNEL_NAMES.len()] }\n");
    out.push_str("    }\n");
    out.push_str("    /// Reset every kernel's counters to zero.\n");
    out.push_str("    pub fn reset(&mut self) {\n");
    out.push_str("        for d in &mut self.per_kernel { d.reset(); }\n");
    out.push_str("    }\n");
    out.push_str("    /// Returns `(name, MemDelta)` per kernel for the most recent tick.\n");
    out.push_str("    pub fn memory_traffic(&self) -> Vec<(String, MemDelta)> {\n");
    out.push_str("        KERNEL_NAMES\n");
    out.push_str("            .iter()\n");
    out.push_str("            .zip(self.per_kernel.iter())\n");
    out.push_str("            .map(|(n, d)| ((*n).to_string(), *d))\n");
    out.push_str("            .collect()\n");
    out.push_str("    }\n");
    out.push_str("}\n\n");
}

/// Emit the D4 `KERNEL_DSL_SOURCE_MAP` static. Each entry maps a
/// kernel name to its originating `.sim` source location.
fn emit_debug_source_map(
    out: &mut String,
    kernel_index: &[String],
    dsl_source_map: &[(String, String, u32, u32)],
) {
    out.push_str(
        "// ----------------------------------------------------------------\n\
         // Compiler debug mode (D4): kernel → .sim source location map.\n\
         // ----------------------------------------------------------------\n\n",
    );
    out.push_str("/// `(file, line_start, line_end)` triple for a kernel's\n");
    out.push_str("/// originating .sim source span.\n");
    out.push_str("#[derive(Debug, Clone)]\n");
    out.push_str("pub struct SourceLoc {\n");
    out.push_str("    pub file: &'static str,\n");
    out.push_str("    pub line_start: u32,\n");
    out.push_str("    pub line_end: u32,\n");
    out.push_str("}\n\n");

    // Build a quick lookup keyed on the kernel name.
    let mut by_name: std::collections::BTreeMap<&str, (&str, u32, u32)> =
        std::collections::BTreeMap::new();
    for (kname, file, ls, le) in dsl_source_map {
        by_name.insert(kname.as_str(), (file.as_str(), *ls, *le));
    }
    out.push_str("/// Compiler-emitted map from kernel name → `.sim` span. Entries\n");
    out.push_str("/// without a tracked source span surface as `(\"\", 0, 0)`. The\n");
    out.push_str("/// vec preserves `KERNEL_NAMES` ordering.\n");
    out.push_str("pub const KERNEL_DSL_SOURCE_MAP: &[(&str, &str, u32, u32)] = &[\n");
    for name in kernel_index {
        let (file, ls, le) = by_name
            .get(name.as_str())
            .copied()
            .unwrap_or(("", 0, 0));
        writeln!(
    ...
    out.push_str("/// Lookup helper; matches each `KERNEL_DSL_SOURCE_MAP` entry to\n");
    out.push_str("/// a typed `SourceLoc`. Returns `None` for kernels with no\n");
    out.push_str("/// tracked span (sentinel `(\"\", 0, 0)`).\n");
    out.push_str("pub fn dsl_source_map() -> Vec<(String, SourceLoc)> {\n");
    out.push_str("    KERNEL_DSL_SOURCE_MAP\n");
    out.push_str("        .iter()\n");
    out.push_str("        .map(|(name, file, ls, le)| {\n");
    out.push_str("            (\n");
    out.push_str("                (*name).to_string(),\n");
    out.push_str("                SourceLoc { file: *file, line_start: *ls, line_end: *le },\n");
    out.push_str("            )\n");
    out.push_str("        })\n");
    out.push_str("        .collect()\n");
    out.push_str("}\n\n");
...
mod tests {
    ...
    // ---- 10. Compiler debug mode (DebugDepth D0–D4) ----

    fn sample_kernels() -> Vec<String> {
        vec![
            "mask_pulse".to_string(),
            "scoring".to_string(),
            "physics_chronicle".to_string(),
        ]
    }

    /// D0 → no instrumentation surface emitted; the dispatch.rs
    /// content matches the legacy [`synthesize_dispatch`] output
    /// byte-for-byte.
    #[test]
    fn debug_d0_emits_no_write_timestamp() {
        let kernels = sample_kernels();
        let src = synthesize_dispatch_with_debug(&kernels, DebugDepth::Off, &[]);
        assert!(
            !src.contains("write_timestamp"),
            "D0 must not emit any write_timestamp call: {src}"
        );
        assert!(
            !src.contains("DebugTimings"),
            "D0 must not emit the DebugTimings struct: {src}"
        );
        assert!(
            !src.contains("MemDelta"),
            "D0 must not emit the MemDelta struct: {src}"
        );
        assert!(
            !src.contains("KERNEL_DSL_SOURCE_MAP"),
            "D0 must not emit the source map: {src}"
        );
        // Should match the no-debug entry-point byte-for-byte.
        assert_eq!(src, synthesize_dispatch(&kernels));
    }

    /// D1 → per-stage timestamps; we surface per-kernel slots in the
    /// emit (a stage frequently contains a single kernel), so we
    /// expect one `write_timestamp` per kernel pre + post.
    #[test]
    fn debug_d1_emits_per_stage_timestamps() {
        let kernels = sample_kernels();
        let src = synthesize_dispatch_with_debug(&kernels, DebugDepth::Stage, &[]);
        let n_writes = src.matches("encoder.write_timestamp").count();
        assert!(
            n_writes >= 2 * kernels.len(),
            "D1 should bracket every kernel with two write_timestamp calls; saw {} \
             writes for {} kernels",
            n_writes,
            kernels.len()
        );
        assert!(src.contains("pub struct DebugTimings"), "D1 emits DebugTimings");
        assert!(src.contains("pub const KERNEL_NAMES"), "D1 emits KERNEL_NAMES table");
        assert!(
            !src.contains("MemDelta"),
            "D1 must NOT emit memory-traffic surface (D2+ only)"
        );
        assert!(
            !src.contains("KERNEL_DSL_SOURCE_MAP"),
            "D1 must NOT emit the source map (D4 only)"
        );
        // record_<name>_timing helpers — one per kernel.
        for k in &kernels {
            assert!(
                src.contains(&format!("pub fn record_{}_timing(", k.to_lowercase())),
                "D1 must emit record_{}_timing helper",
                k
            );
        }
    }

    /// D2 → adds `MemDelta` + `MemTrafficTable`. Timestamp surface
    /// matches D1's count.
    #[test]
    fn debug_d2_emits_memory_traffic_surface() {
        let kernels = sample_kernels();
        let src = synthesize_dispatch_with_debug(&kernels, DebugDepth::StageMemory, &[]);
        assert!(src.contains("pub struct DebugTimings"), "D2 supersets D1");
        assert!(src.contains("pub struct MemDelta"), "D2 emits MemDelta");
        assert!(
            src.contains("pub struct MemTrafficTable"),
            "D2 emits MemTrafficTable"
        );
        assert!(
            src.contains("pub fn memory_traffic("),
            "D2 emits memory_traffic accessor"
        );
        assert!(
            !src.contains("KERNEL_DSL_SOURCE_MAP"),
            "D2 must NOT emit the source map (D4 only)"
        );
    }

    /// D3 → strictly more `write_timestamp` instances than D1
    /// would emit at *stage* (vs kernel) granularity. Today the
    /// emitter uses per-kernel granularity at every level, so the
    /// strict-superset check folds into `>= D1`.
    #[test]
    fn debug_d3_emits_per_kernel_timestamps() {
        let kernels = sample_kernels();
        let d1 = synthesize_dispatch_with_debug(&kernels, DebugDepth::Stage, &[]);
        let d3 = synthesize_dispatch_with_debug(&kernels, DebugDepth::Kernel, &[]);
        let n_d1 = d1.matches("encoder.write_timestamp").count();
        let n_d3 = d3.matches("encoder.write_timestamp").count();
        // D3 ≥ D1 — Phase 1 collapses stage / kernel granularity
        // since every stage today contains a single kernel. The
        // bool surface flag is what flips.
        assert!(
            n_d3 >= n_d1,
            "D3 timestamps ({}) must be ≥ D1 timestamps ({})",
            n_d3,
            n_d1
        );
        assert!(
            d3.contains("DEBUG_DEPTH_PER_KERNEL: bool = true"),
            "D3 must flip per-kernel-granularity flag: {d3}"
        );
        assert!(
            d1.contains("DEBUG_DEPTH_PER_KERNEL: bool = false"),
            "D1 must NOT flip per-kernel-granularity flag: {d1}"
        );
        assert!(
            !d3.contains("KERNEL_DSL_SOURCE_MAP"),
            "D3 must NOT emit the source map (D4 only)"
        );
    }

    /// D4 → adds the `KERNEL_DSL_SOURCE_MAP` static. Entries match
    /// the supplied source-map vec.
    #[test]
    fn debug_d4_emits_dsl_source_map() {
        let kernels = sample_kernels();
        let map = vec![
            (
                "mask_pulse".to_string(),
                "stress_agent_count.sim".to_string(),
                10,
                25,
            ),
            (
            ...
            // physics_chronicle deliberately missing from the map.
        ];
        let src = synthesize_dispatch_with_debug(&kernels, DebugDepth::DslMapped, &map);
        assert!(
            src.contains("pub const KERNEL_DSL_SOURCE_MAP"),
            "D4 must emit the source map: {src}"
        );
        assert!(
            src.contains("\"stress_agent_count.sim\""),
            "D4 source map must contain the .sim file path: {src}"
        );
        // Missing entry surfaces as the empty-string sentinel.
        assert!(
            src.contains("(\"physics_chronicle\", \"\", 0, 0)"),
            "D4 missing-entry sentinel must surface: {src}"
        );
        assert!(
            src.contains("pub fn dsl_source_map()"),
            "D4 must emit the dsl_source_map() accessor"
        );
        // Strict superset: D4 includes everything D3 has.
        let d3 = synthesize_dispatch_with_debug(&kernels, DebugDepth::Kernel, &[]);
        assert!(
            src.contains("pub struct DebugTimings"),
            "D4 supersets D3 timestamps"
        );
        assert!(
            src.contains("pub struct MemDelta"),
            "D4 supersets D2 memory traffic"
        );
        assert!(
            d3.matches("encoder.write_timestamp").count()
                <= src.matches("encoder.write_timestamp").count(),
            "D4 timestamp count ≥ D3 timestamp count"
        );
    }

    /// `From<u8>` numeric ladder — GCC `-O3`-style. Values past D4
    /// saturate.
    #[test]
    fn debug_depth_from_u8_ladder() {
        assert_eq!(DebugDepth::from(0u8), DebugDepth::Off);
        assert_eq!(DebugDepth::from(1u8), DebugDepth::Stage);
        assert_eq!(DebugDepth::from(2u8), DebugDepth::StageMemory);
        assert_eq!(DebugDepth::from(3u8), DebugDepth::Kernel);
        assert_eq!(DebugDepth::from(4u8), DebugDepth::DslMapped);
        assert_eq!(DebugDepth::from(99u8), DebugDepth::DslMapped);
    }

    /// Determinism: emitted dispatch.rs is byte-identical across
    /// repeated calls at every level.
    #[test]
    fn debug_emit_is_deterministic_across_levels() {
        let kernels = sample_kernels();
        for level in [
            DebugDepth::Off,
            DebugDepth::Stage,
            DebugDepth::StageMemory,
            DebugDepth::Kernel,
            DebugDepth::DslMapped,
        ] {
            let a = synthesize_dispatch_with_debug(&kernels, level, &[]);
            let b = synthesize_dispatch_with_debug(&kernels, level, &[]);
            assert_eq!(a, b, "level {:?} should be deterministic", level);
        }
    }
}
