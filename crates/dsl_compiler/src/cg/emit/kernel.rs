...
pub fn kernel_topology_to_spec_and_body(
    ...
    //    (e.g. quest_arc_real's ApplyStageAdvance, which reads
    //    StageAdvanced events and writes mana but emits nothing) with
    //    the Generic `agent_cap`-typed cfg struct — the WGSL then
    //    referenced `cfg.event_count` against a struct that didn't
    //    declare it, surfacing as a naga validation error at runtime.
    //    The fix-comment at line 3596-3598 in this file flagged this
    //    exact case ahead of time. Closing the gap by stamping
    //    PerEventEmit for every PerEvent dispatch (the comment's
    //    recommended fix); the second-pass `event_ring` binding
    //    upgrade below is also required so reads of the consumed
    //    event ring type-check as `array<atomic<u32>>`.
...
/// quest_arc_real Apply-only-PerEvent cfg-mismatch gap), so this
/// helper is currently unreferenced. Kept as defensive scaffolding
/// for the next future call site (e.g. a future kind that needs
/// emit-vs-no-emit discrimination).
#[allow(dead_code)]
...
mod tests {
    ...
    fn spec_kind_matches_topology() {
        ...
        // ApplyStageAdvance) with the Generic `agent_cap`-typed cfg
        // struct — but the WGSL preamble at line 3599 stamps
        // `if (event_idx >= cfg.event_count) { return; }`
        // unconditionally for any PerEvent dispatch, so Generic-cfg'd
        // PerEvent kernels failed naga validation at runtime. The
        // gap was flagged ahead of time in the comment at line 3596.
        // Closing the gap by stamping PerEventEmit for every PerEvent
        // dispatch; this assertion records the new behavior.
