...
// ---------------------------------------------------------------------------
// Per-ability picker emission (Phase 3 — Task A1).
// ---------------------------------------------------------------------------
...
fn lower_ability_score_expr_cpu(expr: &IrExprNode) -> Result<String, EmitError> {
    match &expr.kind {
        IrExpr::LitFloat(f) => Ok(format!("{f:.6}_f32")),
        IrExpr::LitInt(n) => Ok(format!("{n}_i64 as f32")),
        ...
        IrExpr::If { cond, then_expr, else_expr } => {
            ...
            let t = lower_ability_score_expr_cpu(then_expr)?;
            let e = match else_expr {
                Some(inner) => lower_ability_score_expr_cpu(inner)?,
                None => "0.0_f32".to_string(),
            };
