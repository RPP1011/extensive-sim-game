...
///
/// `source_file` is unused for this file — step.rs has a fixed multi-line
/// header comment that identifies the phase pipeline. The parameter is kept
/// for API symmetry with the other single-file emitters.
pub fn emit_step(_source_file: Option<&str>) -> String {
    // The step pipeline is almost entirely static. Emit it verbatim so the
    // committed engine_rules/src/step.rs round-trips through --check.
    r#"// GENERATED by dsl_compiler. Do not edit by hand.
