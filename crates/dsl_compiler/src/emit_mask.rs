...
fn emit_mask_result(
    ...
    // Masks whose predicate references a view get the full 3-line header
    // and doc comments on their fns (they are the "showcase" complex masks).
    // Self-only masks with simple predicates get the terse 1-line header
    // and no doc comments, matching the committed hand-simplified format.
    let complex = predicate_has_view_call(&mask.predicate);
    emit_header(&mut out, source_file, complex);
    emit_imports(&mut out, mask);
    emit_predicate_fn(&mut out, mask, ctx, complex)?;
    ...
    if mask.candidate_source.is_some() {
        ...
        emit_candidate_enumerator_fn(&mut out, mask, ctx, complex)?;
    }
    Ok(out)
}

/// True if the expression tree contains at least one `IrExpr::ViewCall` node.
fn predicate_has_view_call(node: &IrExprNode) -> bool {
    match &node.kind {
        IrExpr::ViewCall(_, _) => true,
        IrExpr::NamespaceCall { args, .. }
        | IrExpr::BuiltinCall(_, args)
        | IrExpr::UnresolvedCall(_, args) => args.iter().any(|a| predicate_has_view_call(&a.value)),
        IrExpr::Binary(_, l, r) => predicate_has_view_call(l) || predicate_has_view_call(r),
        IrExpr::Unary(_, r) => predicate_has_view_call(r),
        _ => false,
    }
}
...
fn emit_header(out: &mut String, source_file: Option<&str>, full: bool) {
    match source_file {
...
fn emit_imports(out: &mut String, mask: &MaskIR) {
    ...
    if mask.candidate_source.is_some() {
        writeln!(out, "use engine::mask::TargetMask;").unwrap();
    }
...
fn emit_candidate_enumerator_fn(
    ...
    if emit_doc {
        writeln!(out, "/// Candidate enumerator.").unwrap();
    }
    writeln!(
        out,
    ...
    writeln!(
        out,
        "{pad}    out.push(self_id, engine::mask::MicroKind::{micro_variant}, {target_binding});"
    )
    .unwrap();
...
fn lower_view_call(
    ...
    match view.kind {
        ViewKind::Lazy => {
            ...
            argv.extend(lowered);
            // Wrap in parens so that `!view_call` in lower_clause produces
            // `!(!(wrapped_call))` matching the committed mask format.
            Ok(format!("(crate::views::{stem}({}))", argv.join(", ")))
        }
...
fn render_head_type(ty: &IrType, mask_name: &str) -> Result<String, EmitError> {
    let rendered = match ty {
        ...
        // `use engine::ids::AgentId;`, so the rarer id types path
        // through their owning module to avoid bloating the import
        // preamble for every mask file.
        IrType::AbilityId => "engine::ability::AbilityId",
        IrType::ItemId => "engine::ids::ItemId",
        IrType::GroupId => "engine::ids::GroupId",
        IrType::QuestId => "engine::ids::QuestId",
        IrType::EventId => "engine::ids::EventId",
        IrType::AuctionId => "engine::ids::AuctionId",
        _ => {
            return Err(EmitError::Unsupported(format!(
...
mod tests {
    ...
    fn attack_mask_emits_expected_shape() {
        ...
        assert!(out.contains("use engine::state::SimState;"));
    ...
    fn view_call_to_lazy_view_emits_crate_generated_path() {
        ...
        assert!(
            out.contains("crate::views::is_hostile(state, self_id, target)"),
