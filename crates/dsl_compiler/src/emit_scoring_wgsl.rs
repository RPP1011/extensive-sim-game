...
use crate::ir::{IrExpr, IrExprNode, ScoringIR};
...
/// V2 emitter: 5-slot stub WGSL matching `emit_scoring_rs_v2`'s
/// 5-slot BGL. No view bindings. Per-agent body writes a deterministic
/// "hold" action into scoring_out. Modernization (full scoring with
/// view reads + atomic argmax) is a follow-up.
pub fn emit_scoring_wgsl_v2() -> String {
    let mut out = String::new();
    out.push_str("// GENERATED by dsl_compiler::emit_scoring_wgsl::emit_scoring_wgsl_v2.\n");
    out.push_str("// 5-slot stub matching emit_scoring_rs_v2's BGL. View reads are TODO.\n");
    out.push_str("\n");
    out.push_str("@group(0) @binding(0) var<storage, read>       agent_data:   array<u32>;\n");
    out.push_str("@group(0) @binding(1) var<storage, read>       mask_bitmaps: array<u32>;\n");
    out.push_str("@group(0) @binding(2) var<storage, read_write> scoring_out:  array<u32>;\n");
    out.push_str("@group(0) @binding(3) var<storage, read>       sim_cfg:      array<u32>;\n");
    out.push_str("struct ScoringCfg { agent_cap: u32, num_actions: u32, tick: u32, _pad: u32 };\n");
    out.push_str("@group(0) @binding(4) var<uniform>             cfg:          ScoringCfg;\n");
    out.push_str("\n");
    out.push_str("// ScoreOutput stride matches engine_gpu/src/sync_helpers.rs.\n");
    out.push_str("const SCORING_STRIDE_U32: u32 = 4u;\n");
    out.push_str("const ACTION_HOLD: u32 = 0u;\n");
    out.push_str("const NO_TARGET: u32 = 0xFFFFFFFFu;\n");
    out.push_str("\n");
    out.push_str("@compute @workgroup_size(64)\n");
    out.push_str("fn cs_scoring(@builtin(global_invocation_id) gid: vec3<u32>) {\n");
    out.push_str("    let i = gid.x;\n");
    out.push_str("    if (i >= cfg.agent_cap) { return; }\n");
    out.push_str("    // Touch every binding so naga keeps slots live.\n");
    out.push_str("    let _a = agent_data[0];\n");
    out.push_str("    let _m = mask_bitmaps[0];\n");
    out.push_str("    let _s = sim_cfg[0];\n");
    out.push_str("    let _t = cfg.tick;\n");
    out.push_str("    // Stub body: write a default 'hold' action per agent.\n");
    out.push_str("    let base = i * SCORING_STRIDE_U32;\n");
    out.push_str("    scoring_out[base + 0u] = ACTION_HOLD;\n");
    out.push_str("    scoring_out[base + 1u] = NO_TARGET;\n");
    out.push_str("    scoring_out[base + 2u] = 0u;\n");
    out.push_str("    scoring_out[base + 3u] = 0u;\n");
    out.push_str("}\n");
    out
}
...
// ---------------------------------------------------------------------------
// `cs_pick_ability` WGSL emission — Subsystem 3 (GPU ability evaluation).
// ---------------------------------------------------------------------------
//
// Companion to `emit_scoring::emit_pick_ability_cpu`. Lowers a `per_ability`
// scoring row to a stand-alone WGSL compute kernel `cs_pick_ability` that
// runs one thread per agent, walks ability slots, scores them via
// `eval_ability_score`, and writes the (ability_slot, target_slot) tuple
// into `chosen_ability_buf`. The shared `SimCfg` struct + binding(5) is
// emitted via `emit_sim_cfg_struct_wgsl` so layout stays in sync with the
// CPU and other GPU kernels.
...
pub fn emit_pick_ability_wgsl(scoring: &ScoringIR) -> String {
    ...
    // Shared SimCfg struct + binding(5). Single source of truth for the
    // layout — see `emit_sim_cfg.rs`.
    emit_sim_cfg_struct_wgsl(&mut out, 5);

    writeln!(out, "struct PickAbilityAgent {{").unwrap();
    writeln!(out, "    pos_x: f32,").unwrap();
    writeln!(out, "    pos_y: f32,").unwrap();
    writeln!(out, "    pos_z: f32,").unwrap();
    writeln!(out, "    alive: u32,").unwrap();
    writeln!(out, "    creature_type: u32,").unwrap();
    writeln!(out, "    _pad0: u32,").unwrap();
    writeln!(out, "    _pad1: u32,").unwrap();
    ...
    // No-cast sentinel — high u32 in both halves of the (slot,target) pair.
    writeln!(out, "const SENTINEL_NO_CAST: u32 = 0xFFFFFFFFu;").unwrap();
    writeln!(out, "const MAX_ABILITIES: u32 = 256u;").unwrap();
    writeln!(out, "const PICK_ABILITY_WORKGROUP_SIZE: u32 = 64u;").unwrap();
    writeln!(out).unwrap();

    writeln!(
        out,
        "@group(0) @binding(0) var<storage, read> agents: array<PickAbilityAgent>;"
    )
    .unwrap();
    writeln!(
        out,
        "@group(0) @binding(1) var<storage, read> ability_registry: array<GpuPickAbilityEntry>;"
    )
    .unwrap();
    writeln!(
        out,
        "@group(0) @binding(2) var<storage, read> per_slot_cooldown: array<u32>;"
    )
    .unwrap();
    writeln!(
        out,
        "@group(0) @binding(3) var<storage, read_write> chosen_ability_buf: array<u32>;"
    )
    .unwrap();
    writeln!(out, "// chosen_ability_buf packs two u32s per agent:").unwrap();
    writeln!(
        out,
        "//   [agent*2+0] = ability_slot (SENTINEL_NO_CAST = no cast)"
    )
    .unwrap();
    writeln!(
        out,
        "//   [agent*2+1] = target_agent_slot (SENTINEL_NO_CAST = no target)"
    )
    .unwrap();
    writeln!(
        out,
        "@group(0) @binding(4) var<uniform> cfg: PickAbilityCfg;"
    )
    .unwrap();
    // sim_cfg @binding(5) emitted by emit_sim_cfg_struct_wgsl above.
    writeln!(
        out,
        "@group(0) @binding(6) var<storage, read> tag_values: array<f32>;"
    )
    .unwrap();
    writeln!(out).unwrap();

    // Helper: tag_value(ab_slot, tag_idx)
    writeln!(out, "fn tag_value(ab_slot: u32, tag_idx: u32) -> f32 {{").unwrap();
    writeln!(out, "    let idx = ab_slot * cfg.num_tags + tag_idx;").unwrap();
    writeln!(
        out,
        "    if (idx < arrayLength(&tag_values)) {{ return tag_values[idx]; }}"
    )
    .unwrap();
    writeln!(out, "    return 0.0;").unwrap();
    writeln!(out, "}}").unwrap();
    ...
    writeln!(
        out,
        "    return agents[a].alive != 0u && agents[b].alive != 0u"
    )
    .unwrap();
    writeln!(
        out,
        "        && agents[a].creature_type != agents[b].creature_type;"
    )
    .unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    ...
    // Emit score function body from the (single) per_ability row.
    writeln!(
        out,
        "fn eval_ability_score(agent_slot: u32, ab_slot: u32) -> f32 {{"
    )
    .unwrap();
    if let Some(row) = scoring.per_ability_rows.first() {
    ...
    writeln!(
        out,
        "fn cs_pick_ability(@builtin(global_invocation_id) gid: vec3<u32>) {{"
    )
    .unwrap();
    writeln!(out, "    let agent_slot = gid.x;").unwrap();
    ...
    writeln!(
        out,
        "        chosen_ability_buf[agent_slot * 2u] = SENTINEL_NO_CAST;"
    )
    .unwrap();
    writeln!(
        out,
        "        chosen_ability_buf[agent_slot * 2u + 1u] = SENTINEL_NO_CAST;"
    )
    .unwrap();
    writeln!(out, "        return;").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "    var best_score: f32 = -1.0e38;").unwrap();
    writeln!(out, "    var best_ab: u32 = SENTINEL_NO_CAST;").unwrap();
    writeln!(out, "    var best_tgt: u32 = SENTINEL_NO_CAST;").unwrap();
    writeln!(out, "    let tick = cfg.tick;").unwrap();
    writeln!(
        out,
        "    for (var ab_slot: u32 = 0u; ab_slot < cfg.ability_count; ab_slot = ab_slot + 1u) {{"
    )
    .unwrap();
    writeln!(
        out,
        "        if (ability_registry[ab_slot].known == 0u) {{ continue; }}"
    )
    .unwrap();
    // guard: per-slot cooldown
    writeln!(out, "        // guard: per-slot cooldown").unwrap();
    writeln!(
        out,
        "        let cd_idx = agent_slot * MAX_ABILITIES + ab_slot;"
    )
    .unwrap();
    writeln!(
        out,
        "        if (cd_idx < arrayLength(&per_slot_cooldown)) {{"
    )
    .unwrap();
    writeln!(
        out,
        "            if (per_slot_cooldown[cd_idx] > tick) {{ continue; }}"
    )
    .unwrap();
    writeln!(out, "        }}").unwrap();
    // score
    writeln!(
        out,
        "        let score = eval_ability_score(agent_slot, ab_slot);"
    )
    .unwrap();
    ...
    writeln!(
        out,
        "        let range = ability_registry[ab_slot].range_bits;"
    )
    .unwrap();
    writeln!(out, "        var nearest_tgt: u32 = SENTINEL_NO_CAST;").unwrap();
    writeln!(out, "        var nearest_dist: f32 = 1.0e38;").unwrap();
    writeln!(
        out,
        "        for (var t: u32 = 0u; t < cfg.agent_cap; t = t + 1u) {{"
    )
    .unwrap();
    writeln!(out, "            if (t == agent_slot) {{ continue; }}").unwrap();
    writeln!(
        out,
        "            if (!is_hostile(agent_slot, t)) {{ continue; }}"
    )
    .unwrap();
    writeln!(out, "            let d = dist3(agent_slot, t);").unwrap();
    writeln!(
        out,
        "            if (d <= range && d < nearest_dist) {{"
    )
    .unwrap();
    ...
    writeln!(
        out,
        "        if (nearest_tgt == SENTINEL_NO_CAST) {{ continue; }}"
    )
    .unwrap();
    ...
    writeln!(
        out,
        "    chosen_ability_buf[agent_slot * 2u + 1u] = best_tgt;"
    )
    .unwrap();
    writeln!(out, "}}").unwrap();
...
/// Lower a `per_ability` row's score expression into WGSL source.
///
/// Mirrors `emit_scoring::lower_ability_score_expr_cpu` — the supported
/// shape set is intentionally narrow (literals, ability accessors,
/// arithmetic binops, `if/else`). Anything outside this set returns a
/// `0.0 /* unsupported */` placeholder so a partial lowering doesn't
/// silently break parity in test fixtures that only assert on shape.
fn lower_ability_score_expr_wgsl(expr: &IrExprNode) -> String {
    match &expr.kind {
        // The IR splits literals into LitFloat/LitInt; there is no
        // generic `Lit(f)` variant. Emit floats with 6 fractional digits
        // and ints as `<n>.0` so the WGSL value is f32.
        IrExpr::LitFloat(f) => format!("{f:.6}"),
        IrExpr::LitInt(n) => format!("{n}.0"),
        ...
        // The real IR uses `then_expr` / `else_expr` (with `else_expr`
        // optional) — not `then_val` / `else_val`. A missing `else`
        // lowers to `0.0`, mirroring the CPU emitter.
        IrExpr::If {
            cond,
            then_expr,
            else_expr,
            ...
            let t = lower_ability_score_expr_wgsl(then_expr);
            let e = match else_expr {
                Some(inner) => lower_ability_score_expr_wgsl(inner),
                None => "0.0".to_string(),
            };
