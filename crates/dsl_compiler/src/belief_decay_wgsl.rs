//! Wave 3 ToM Phase 3.8 — generated WGSL for the BeliefState
//! per-tick decay sweep.
//!
//! ## Why this lives in the compiler crate (not the runtime)
//!
//! The user's "no hand-written WGSL in runtime crates" constraint
//! (Phase 3.8 retirement of the four `OBSERVE_WGSL` / `SCRY_WGSL` /
//! `REVEAL_WGSL` / `DECAY_WGSL` const strings in
//! `tom_probe_runtime/src/lib.rs`) requires that any WGSL the
//! runtime dispatches be either compiler-emitted from a `.sim`
//! source OR generated by a function in the compiler crate.
//!
//! Observe / scry / reveal flipped to .sim-authored consumer rules
...
//! rule walk all cells. Adding such a primitive is a separate slice;
//! meanwhile the WGSL still must come from the compiler, not from a
