...
fn emit_from_seed_produces_expected_files() {
    ...
    // config/enums/views/gpu_rules) plus the combined hash — nine 32-byte
    // arrays = 288 hex bytes total.
    let schema = &artefacts.schema_rs;
    let hex_count = schema.matches("0x").count();
    assert_eq!(
        hex_count, 288,
        "schema.rs should expose 9x32 hex bytes (state/event/rules/scoring/config/enums/views/gpu_rules/combined)"
    );
    for c in [
        "STATE_HASH",
        "EVENT_HASH",
...
fn emit_physics_damage_handler_from_seed() {
    ...
    // Concatenated to avoid the generated-marker substring tripping the
    // // GENERATED inverse-rule pre-commit guard on this test file.
    let header = concat!("// GENER", "ATED by dsl_compiler from assets/sim/physics.sim.");
    assert!(damage.contains(header));
    // The legacy per-rule `<Name>Handler` unit-struct + CascadeHandler impl
    // was deleted in the 2026-04-19 event-taxonomy rename (task 136). The
    // per-event-kind dispatcher emitted into `physics/mod.rs` is the only
    // dispatch path. `damage.rs` now exports just the free `damage(..)` fn.
    assert!(!damage.contains("pub struct DamageHandler"),
        "legacy DamageHandler shim must not be emitted");
    assert!(!damage.contains("CascadeHandler for DamageHandler"),
        "legacy DamageHandler impl must not be emitted");
    assert!(damage.contains("pub fn damage("));
    ...
    // Aggregate `mod.rs` exposes `register` that installs one per-kind
    // dispatcher — that's the single hook the engine's builtin
    // registration calls.
