//! `tower_defense_runtime` build script. Mirrors `duel_1v1_runtime`'s
//! build.rs verbatim except for the input fixture path.
//!
//! Lowers `assets/sim/tower_defense.sim` through the DSL compiler
//! pipeline (parse → resolve → CG lower → schedule → emit) and writes
//! the per-kernel WGSL + Rust files into `OUT_DIR/<kernel>.{wgsl,rs}`,
//! concatenated into `OUT_DIR/generated.rs` for `include!` into
//! `src/lib.rs`. Tolerates lower diagnostics — tower_defense reuses
//! the same physics-cycle and post-phase shapes the duel surfaced
//! (well_formed P6 + cycle warnings) which are advisory not blocking.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/tower_defense_runtime");
    let sim_path = workspace_root.join("assets/sim/tower_defense.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse tower_defense.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve tower_defense.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[tower_defense lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit tower_defense CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[tower_defense emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[tower_defense emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by tower_defense_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/tower_defense.sim and rebuilding.\n\n",
    );
