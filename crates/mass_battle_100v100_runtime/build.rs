//! `mass_battle_100v100_runtime` build script. Mirrors
//! `duel_1v1_runtime/build.rs` shape verbatim except for the input
//! fixture path.
//!
//! Lowers `assets/sim/mass_battle_100v100.sim` through the DSL
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics — same well_formed P6 + cycle
//! warnings as duel_1v1 (chronicle physics writes agent.hp/alive +
//! Snipe scoring reads agent.hp on the same ring) which are
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/mass_battle_100v100_runtime");
    let sim_path = workspace_root.join("assets/sim/mass_battle_100v100.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse mass_battle_100v100.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve mass_battle_100v100.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[mass_battle_100v100 lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit mass_battle_100v100 CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[mass_battle_100v100 emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[mass_battle_100v100 emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by mass_battle_100v100_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/mass_battle_100v100.sim and rebuilding.\n\n",
    );
