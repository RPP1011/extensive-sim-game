//! `objective_capture_10v10_runtime` build script. Mirrors
//! `duel_1v1_runtime`'s build.rs verbatim except for the input
//! fixture path.
//!
//! Lowers `assets/sim/objective_capture_10v10.sim` through the DSL
//! compiler pipeline (parse → resolve → CG lower → schedule →
//! emit). The resulting WGSL + Rust files land in
//! OUT_DIR/<kernel>.{wgsl,rs} and are concatenated into
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/objective_capture_10v10_runtime");
    let sim_path = workspace_root.join("assets/sim/objective_capture_10v10.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse objective_capture_10v10.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve objective_capture_10v10.sim");
    // Tolerate lower diagnostics — same advisory P6/cycle warnings
    // as duel_1v1 since the chronicle physics writes hp/alive on
    // the same ring scoring reads.
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[obj_cap lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit objective_capture_10v10 CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[obj_cap emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[obj_cap emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by objective_capture_10v10_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/objective_capture_10v10.sim and rebuilding.\n\n",
    );
