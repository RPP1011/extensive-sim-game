...
use crate::effects::{AbilityTarget, AbilityTargeting, Area, Effect};

use crate::squad::personality::Personality;
use crate::squad::state::{personality_movement_profile, FormationMode, TickContext};
...
pub(crate) fn evaluate_hero_ability(
    ...
    for (i, slot) in unit.abilities.iter().enumerate() {
        ...
        let damage_from_effects = |effects: &[crate::effects::ConditionalEffect]| -> f32 {
            effects.iter()
        ...
        let met_conditional_damage: f32 = slot.def.effects.iter()
            ...
            .filter_map(|ce| {
                let cond_met = match &ce.condition {
                    Some(crate::effects::Condition::TargetIsStunned) => target_is_controlled,
                    Some(crate::effects::Condition::TargetIsSlowed) => {
                        target.status_effects.iter().any(|s| matches!(s.kind, crate::effects::StatusKind::Slow { .. }))
                    }
                    Some(crate::effects::Condition::TargetHpBelow { percent }) => {
...
pub(in crate::squad) fn choose_action(
    state: &SimState,
    unit_id: u32,
    target_id: u32,
