...
fn bias_hero_move(nav: Option<&GridNav>, unit: &crate::sim::UnitState, next_pos: SimVec2, max_step: f32, enemy_centroid: Option<SimVec2>) -> SimVec2 {
    match nav {
        Some(n) if unit.team == Team::Hero => {
            crate::pathing::terrain_biased_step(n, unit.position, next_pos, max_step, enemy_centroid)
        }
        _ => next_pos,
    }
}
