    ...
    for (tick, intents) in script.iter().enumerate() {
        ...
        for intent in intents {
            ...
            let target = match intent.action {
                crate::sim::IntentAction::Attack { target_id } => Some(target_id),
                crate::sim::IntentAction::CastAbility { target_id } => Some(target_id),
                _ => None,
            };
...
fn squad_mode_changes_over_time() {
    ...
    let mut hero_modes = run
        .board_history
        .iter()
        .filter_map(|m| m.get(&crate::sim::Team::Hero).map(|b| b.mode))
        .collect::<Vec<_>>();
    hero_modes.dedup();
...
fn phase3_multi_seed_metrics_bands() {
    ...
    for seed in seeds {
        ...
        let replay = crate::sim::run_replay(initial.clone(), &script, 200, FIXED_TICK_MS);
        if replay.metrics.winner == Some(crate::sim::Team::Hero) {
            hero_wins += 1;
        }
...
fn tie_break_target_selection_is_deterministic() {
    let mut state = sample_phase3_party_state(55);
    {
        ...
        enemy4.hp = 100;
        enemy4.position = crate::sim::sim_vec2(8.0, 0.0);
    }
    {
        let enemy5 = state.units.iter_mut().find(|u| u.id == 5).unwrap();
        enemy5.hp = 100;
        enemy5.position = crate::sim::sim_vec2(8.0, 0.0);
    }
    ...
    let pick_targets = |intents: &[crate::sim::UnitIntent]| -> HashSet<u32> {
        intents
            .iter()
...
fn no_enemies_produces_hold_for_entire_squad() {
    ...
    state.units.retain(|u| u.team == crate::sim::Team::Hero);
    let mut ai = SquadAiState::new_inferred(&state);
    let intents = generate_intents(&state, &mut ai, FIXED_TICK_MS);
    assert!(intents
        .iter()
        .all(|intent| matches!(intent.action, crate::sim::IntentAction::Hold)));
}
...
fn hash_changes_with_small_parameter_mutation() {
    ...
    let mutated = crate::sim::run_replay(initial, &script, 280, FIXED_TICK_MS);
...
fn personality_inference_ranges() {
    use crate::sim::sim_vec2;

    // Tanky melee -- high aggression, low caution
    let mut tank = crate::sim::UnitState {
        id: 1, team: crate::sim::Team::Hero, hp: 180, max_hp: 180,
...
fn attack_dominates_during_approach() {
    ...
    let unit = initial.units.iter().find(|u| u.team == crate::sim::Team::Hero).unwrap();
