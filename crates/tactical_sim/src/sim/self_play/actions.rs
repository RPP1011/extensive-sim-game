        ...
        if let Some(slot) = unit.abilities.get(i) {
            ...
            if ready {
                let has_target = match slot.def.targeting {
                    crate::effects::AbilityTargeting::TargetEnemy => has_enemies,
                    crate::effects::AbilityTargeting::TargetAlly => {
...
pub fn intent_to_action(
    ...
    match intent {
        IntentAction::Attack { target_id } => {
            let enemies: Vec<&crate::sim::UnitState> = state.units.iter()
...
pub fn action_to_intent_with_focus(
    ...
    let enemies: Vec<&crate::sim::UnitState> = state.units.iter()
        .filter(|u| u.team != unit.team && is_alive(u))
        .collect();
    let allies: Vec<&crate::sim::UnitState> = state.units.iter()
        .filter(|u| u.team == unit.team && is_alive(u))
        .collect();
        ...
        a @ 3..=10 => {
            ...
            if let Some(slot) = unit.abilities.get(ability_index) {
                let target = match slot.def.targeting {
                    crate::effects::AbilityTargeting::TargetEnemy => {
                    ...
                    crate::effects::AbilityTargeting::GroundTarget => {
                        if !enemies.is_empty() {
