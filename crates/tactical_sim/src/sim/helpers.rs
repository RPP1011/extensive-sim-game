...
use contracts::*;
use crate::effects::{ScalingTerm, StatRef, StatusKind};
...
pub fn can_see(observer: &UnitState, target: &UnitState) -> bool {
    let is_stealthed = target.status_effects.iter()
        .any(|s| matches!(s.kind, crate::effects::StatusKind::Stealth { .. }));
    if !is_stealthed {
        return true;
    }
...
pub fn can_see_with_nav(
    ...
    if !crate::pathing::has_line_of_sight(nav, observer.position, target.position) {
        return false;
    }
...
pub fn move_towards_position(
    ...
    let next = if let Some(ref nav) = state.grid_nav {
        ...
        if nav.is_walkable_pos(direct) {
            ...
            let waypoint = crate::pathing::next_waypoint(nav, start, target_pos);
            let raw = move_towards(start, waypoint, max_delta);
            crate::pathing::clamp_step_to_walkable(nav, start, raw)
        }
    } else {
...
pub fn check_tags_resisted(effect_tags: &crate::effects::Tags, resistance_tags: &crate::effects::Tags) -> bool {
