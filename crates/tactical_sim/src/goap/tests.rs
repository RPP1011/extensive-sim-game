...
fn verify_detects_excessive_replanning() {
    ...
    let state = crate::sim::SimState {
        tick: 10,
        rng_state: 0,
        units: crate::sim::UnitStore::new(vec![crate::sim::UnitState {
            id: 1,
            team: crate::sim::Team::Hero,
            hp: 100,
            max_hp: 100,
            position: crate::sim::SimVec2 { x: 0.0, y: 0.0 },
...
fn bench_world_state_extract() {
    ...
    for i in 0..8 {
        units.push(crate::sim::UnitState {
            id: i,
            team: if i < 4 { crate::sim::Team::Hero } else { crate::sim::Team::Enemy },
            hp: 100,
            max_hp: 100,
            position: crate::sim::SimVec2 { x: i as f32 * 2.0, y: 5.0 },
            move_speed_per_sec: 3.0,
            attack_damage: 10,
            attack_range: 1.5,
            attack_cooldown_ms: 1000,
    ...
    let state = crate::sim::SimState {
        tick: 100,
        rng_state: 0,
        units: crate::sim::UnitStore::new(units),
        projectiles: vec![],
        passive_trigger_depth: 0,
        zones: vec![],
        tethers: vec![],
