...
use crate::sim::{run_replay, sim_vec2, step, SimState, Team, UnitIntent, UnitState, UnitStore};
use crate::pathing::GridNav;
...
fn custom_scenario_intents(state: &SimState, nav: &GridNav, dt_ms: u32) -> Vec<UnitIntent> {
    crate::advanced::build_environment_reactive_intents(state, nav, dt_ms)
}
...
pub(crate) fn build_phase5_event_visualization_html(seed: u64, ticks: u32) -> String {
    let run =
        crate::personality::run_phase5_sample(seed, ticks, crate::sim::FIXED_TICK_MS);
    let replay = run.replay;
    let event_rows = build_event_rows(&replay);

    let initial = sample_phase5_party_state(seed);
    let roles = crate::roles::default_roles();
    ...
    let frame_rows = build_frame_rows(&initial, &script, crate::sim::FIXED_TICK_MS);
...
pub fn export_horde_chokepoint_visualization(path: &str, seed: u64, ticks: u32) -> io::Result<()> {
    let (initial, script) = crate::advanced::build_horde_chokepoint_script(
        seed,
        ticks,
        crate::sim::FIXED_TICK_MS,
    );
    ...
    let frame_rows = build_frame_rows(&initial, &script, crate::sim::FIXED_TICK_MS);
    let event_rows = build_event_rows(&replay);
    let nav = crate::advanced::horde_chokepoint_nav();
...
pub fn export_horde_chokepoint_hero_favored_visualization(
    ...
    let (initial, script) = crate::advanced::build_horde_chokepoint_hero_favored_script(
        seed,
        ticks,
        crate::sim::FIXED_TICK_MS,
    );
...
pub fn export_custom_scenario_visualization(
    ...
    let scenario: CustomScenario = serde_json::from_str(&text)
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e.to_string()))?;
    let (initial, script) = build_custom_scenario_script(&scenario, crate::sim::FIXED_TICK_MS);
    let replay = run_replay(
        initial.clone(),
        &script,
        scenario.ticks,
