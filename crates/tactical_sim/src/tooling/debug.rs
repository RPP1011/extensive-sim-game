...
use crate::sim::{distance, move_away, position_at_range, IntentAction, SimState, UnitState};
...
pub fn build_phase5_debug(seed: u64, ticks: u32, top_k: usize) -> Vec<TickDecisionDebug> {
    ...
    for tick in 0..ticks as usize {
        ...
        let (new_state, _) = crate::sim::step(state, &intents, crate::sim::FIXED_TICK_MS);
        state = new_state;
    }
