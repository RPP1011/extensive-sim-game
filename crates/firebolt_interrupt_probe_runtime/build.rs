//! `firebolt_interrupt_probe_runtime` build script — Plan G G2.5.
//!
//! Lowers `assets/sim/firebolt_interrupt_probe.sim` through the DSL
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics — both `ApplyChronicleDamage` and
//! `InterruptCastOnDamage` are PerEvent consumers that mutate agent
//! SoA, which trips the same P6 `well_formed` diagnostic the
//! firebolt_probe runtime tolerates. Mirrors firebolt_probe_runtime's
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/firebolt_interrupt_probe_runtime");
    let sim_path = workspace_root.join("assets/sim/firebolt_interrupt_probe.sim");
    ...
    // Tolerate lower diagnostics — the consumer rules
    // (ApplyChronicleDamage, InterruptCastOnDamage) trip P6
    // (PerEvent + agents.set_*). Mirror firebolt_probe_runtime's
    // build.rs: surface the diag, take `o.program` anyway.
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[firebolt_interrupt_probe lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit firebolt_interrupt_probe CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[firebolt_interrupt_probe emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[firebolt_interrupt_probe emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by firebolt_interrupt_probe_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/firebolt_interrupt_probe.sim and rebuilding.\n\n",
    );
