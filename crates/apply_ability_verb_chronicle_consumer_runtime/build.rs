//! `apply_ability_verb_chronicle_consumer_runtime` build script.
//!
//! Lowers `assets/sim/apply_ability_verb_chronicle_consumer.sim` through the
//! DSL compiler pipeline (parse → resolve → CG lower → schedule → emit).
//! The resulting WGSL + Rust files land in `OUT_DIR/<kernel>.{wgsl,rs}` and
//! are concatenated into `OUT_DIR/generated.rs` for `include!` into
//! `src/lib.rs`.
//!
//! Tolerates lower diagnostics — the consumer rule trips the same P6
//! well_formed diagnostic that `duel_abilities.sim::ApplyDamage` and
...
//! kernels.
//!
//! ## Closed-loop kind tag (engine-aliased; fixed 2026-05-06)
//!
//! The dispatcher writes `EffectDamageApplied` records with the engine's
//! hardcoded `EventKindId::EffectDamageApplied = 26`. Engine event names
//! are aliased to their hardcoded discriminants at resolve time — see
//! `dsl_ast::engine_events::engine_event_kind_id_for_name`. The
//! consumer's kind-filter emits `if (kind == 26u)` directly; no
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/apply_ability_verb_chronicle_consumer_runtime");
    let sim_path =
        workspace_root.join("assets/sim/apply_ability_verb_chronicle_consumer.sim");
    ...
    let program = dsl_compiler::parse(&src)
        .expect("parse apply_ability_verb_chronicle_consumer.sim");
    let comp = dsl_ast::resolve::resolve(program)
        .expect("resolve apply_ability_verb_chronicle_consumer.sim");
    // Tolerate lower diagnostics — the consumer rule trips P6 (PerEvent +
    // agents.set_hp). Mirror `apply_ability_chronicle_consumer_runtime/
    // build.rs` and `duel_abilities_runtime/build.rs`: surface the diag,
    // take `o.program` anyway.
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!(
                    "cargo:warning=[apply_ability_verb_chronicle_consumer lower diag] {d}"
                );
            }
            o.program
        }
    ...
    let artifacts =
        dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
            .expect("emit apply_ability_verb_chronicle_consumer CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[apply_ability_verb_chronicle_consumer emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[apply_ability_verb_chronicle_consumer emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by apply_ability_verb_chronicle_consumer_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/apply_ability_verb_chronicle_consumer.sim and rebuilding.\n\n",
    );
