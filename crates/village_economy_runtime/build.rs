//! `village_economy_runtime` build script. Mirrors `spy_network_runtime`'s
//! build.rs verbatim except for the input fixture path and the .ability
//! corpus filenames. Lowers `assets/sim/village_economy.sim` through
//! the DSL compiler pipeline (parse → resolve → CG lower → schedule →
//! emit). The resulting WGSL + Rust files land in OUT_DIR/<kernel>.{wgsl,rs}
//! and are concatenated into `OUT_DIR/generated.rs` for `include!` into
//! `src/lib.rs`.
//!
//! Tolerates lower diagnostics — village_economy inherits spy_network's
//! known-deferred well_formed warnings (P6 + cycle: chronicle physics
//! writes agent.hp / mana / hunger on the same ring the verb scoring
//! reads).
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/village_economy_runtime");
    let sim_path = workspace_root.join("assets/sim/village_economy.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse village_economy.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve village_economy.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[village_economy lower diag] {d}");
            }
            o.program
        }
    };
    ...
    println!(
        "cargo:warning=[village_economy emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[village_economy emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by village_economy_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/village_economy.sim and rebuilding.\n\n",
    );
