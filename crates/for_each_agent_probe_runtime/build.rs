//! `for_each_agent_probe_runtime` build script. Mirrors
//! `target_chaser_runtime`'s build.rs shape verbatim except for the
//! input fixture path.
//!
//! Lowers `assets/sim/for_each_agent_probe.sim` through the DSL
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/for_each_agent_probe_runtime");
    let sim_path = workspace_root.join("assets/sim/for_each_agent_probe.sim");
    ...
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        ...
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[for_each_agent_probe lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit for_each_agent_probe CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[for_each_agent_probe emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[for_each_agent_probe emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by for_each_agent_probe_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/for_each_agent_probe.sim and rebuilding.\n\n",
    );
