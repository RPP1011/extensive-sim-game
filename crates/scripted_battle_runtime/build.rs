//! `scripted_battle_runtime` build script. Mirrors `duel_1v1_runtime`'s
//! build.rs shape verbatim except for the input fixture path.
//!
//! Lowers `assets/sim/scripted_battle.sim` through the DSL compiler
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics — scripted_battle inherits duel_1v1's
//! P6 + cycle warnings (chronicle physics writes agent.hp/alive +
//! Phase-1/3 verbs read agent.hp on the same ring) which are
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/scripted_battle_runtime");
    let sim_path = workspace_root.join("assets/sim/scripted_battle.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse scripted_battle.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve scripted_battle.sim");
    // Tolerate lower diagnostics — same well_formed warnings as
    // duel_1v1 (P6 + cycle) that don't block emit.
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[scripted_battle lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit scripted_battle CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[scripted_battle emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[scripted_battle emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by scripted_battle_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/scripted_battle.sim and rebuilding.\n\n",
    );
