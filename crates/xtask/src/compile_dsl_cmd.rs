    ...
    let _c = cfg.agent_cap;\n\
        ...
        // ----- Per-kernel emit block: ScoringKernel + ScoringUnpackKernel (Task 6).
        {
            ...
            use dsl_compiler::emit_scoring_kernel::{
                emit_scoring_unpack_rs, ViewSpecForEmit,
            };
            use dsl_compiler::emit_transient_handles::TransientField;
            ...
            let _ = &wgsl_specs; // legacy emit_scoring_wgsl_atomic_views argument
            let scoring_wgsl_body = dsl_compiler::emit_scoring_wgsl::emit_scoring_wgsl_v2();
...
fn cs_fold_{name}(@builtin(global_invocation_id) gid: vec3<u32>) {{\n\
    ...
    let _c = cfg.tick;\n\
        ...
        if args.cg_canonical {
            ...
            let mut artifacts = match emit_cg_program(&synthesis.schedule, &prog) {
                Ok(a) => a,
                Err(e) => {
            ...
            // ----- Route C splice — scoring + movement legacy emitters.
            //
            // CHECKPOINT, NOT FINAL CG-CANONICAL STATE. Today CG lowers
            // masks, folds, physics chronicles, spatial, and plumbing
            // (26 kernels) but does NOT lower Scoring (PerUnit + view
            // reads) or Movement-as-rule. CPU's UtilityBackend runs the
            // full step chain (scoring → apply_actions → movement) and
            // mutates agent positions; under --cg-canonical the GPU
            // SCHEDULE has no movement → agents stay at spawn → parity
            // diverges on `pos_x_bits`/`pos_z_bits` after t1+ ticks.
            //
            // Route C closes that gap by splicing the legacy
            // `emit_scoring_kernel` + `emit_movement_kernel` outputs
            // into the CG-emitted artifacts. The proper closure is
            // Phase 6 work (Route A): extending CG lowering for PerUnit
            // + Positional + introducing a Movement-as-rule IR variant.
            // Once Route A lands, this splice block deletes.
            //
            // Strategy: push `"scoring"` and `"movement"` into
            // `kernel_index`, insert the WGSL + RS file contents, then
            // re-synthesize lib.rs + dispatch.rs (which walk
            // `kernel_index`) and post-process schedule.rs (which walks
            // schedule.stages, NOT kernel_index — cleanest fix is a
            // string injection right before the closing `];`).
            //
            // The legacy emitters reference `crate::fused_mask::bgl_*`
            // for BGL helpers (legacy crate has its own `fused_mask`
            // module exporting them at module scope). The CG-synthesized
            // `lib.rs` exposes them at crate scope as `crate::bgl_*`,
            // and the CG pipeline does NOT emit a `fused_mask` module
            // (only `fused_mask_Flee`). String-rewrite the helper paths
            // so the spliced kernels resolve against the CG `lib.rs`
            // surface.
            //
            // Known limitation (acknowledged checkpoint state):
            // `emit_scoring_wgsl_v2` is a 5-slot stub that writes
            // ACTION_HOLD for every agent (no view reads, no utility
            // scoring). Movement only mutates position on
            // ACTION_MOVE_TOWARD or ACTION_FLEE. So this splice may
            // close some but not all of the parity divergence — the
            // test `parity_with_cpu_n4_t{1,10,100}` becomes a
            // regression catcher for future Route A work even if
            // currently not green for all three. See plan
            // `docs/superpowers/plans/2026-05-01-cg-lowering-gap-closure.md`.
            {
                use dsl_compiler::cg::emit::cross_cutting::synthesize_dispatch;
                use dsl_compiler::cg::emit::program::synthesize_lib_rs;

                fn rewrite_legacy_bgl_paths(s: String) -> String {
                    // Legacy emitters reference `crate::fused_mask::bgl_*`;
                    // CG `lib.rs` exposes the same helpers at crate scope.
                    s.replace("crate::fused_mask::bgl_storage", "crate::bgl_storage")
                        .replace("crate::fused_mask::bgl_uniform", "crate::bgl_uniform")
                }

                let scoring_rs = rewrite_legacy_bgl_paths(
                    dsl_compiler::emit_scoring_kernel::emit_scoring_rs_v2(),
                );
                let scoring_wgsl_body =
                    dsl_compiler::emit_scoring_wgsl::emit_scoring_wgsl_v2();
                let scoring_wgsl = format!(
                    "// GENERATED by dsl_compiler::emit_scoring_wgsl. \
                     Do not edit by hand.\n{scoring_wgsl_body}"
                );
                let movement_rs = rewrite_legacy_bgl_paths(
                    dsl_compiler::emit_movement_kernel::emit_movement_rs(),
                );
                let movement_wgsl_body =
                    dsl_compiler::emit_movement_wgsl::emit_movement_wgsl();
                let movement_wgsl = format!(
                    "// GENERATED by dsl_compiler::emit_movement_wgsl. \
                     Do not edit by hand.\n{movement_wgsl_body}"
                );

                artifacts
                    .wgsl_files
                    .insert("scoring.wgsl".to_string(), scoring_wgsl);
                artifacts
                    .rust_files
                    .insert("scoring.rs".to_string(), scoring_rs);
                artifacts
                    .wgsl_files
                    .insert("movement.wgsl".to_string(), movement_wgsl);
                artifacts
                    .rust_files
                    .insert("movement.rs".to_string(), movement_rs);

                // Push to kernel_index so re-synthesized lib.rs +
                // dispatch.rs include scoring / movement modules,
                // enum variants, KernelCache fields, and dispatch arms.
                artifacts.kernel_index.push("scoring".to_string());
                artifacts.kernel_index.push("movement".to_string());

                // Re-synthesize lib.rs + dispatch.rs against the
                // updated kernel_index. (synthesize_schedule walks
                // schedule.stages, not kernel_index — handled below
                // via string injection.)
                artifacts.rust_files.insert(
                    "lib.rs".to_string(),
                    synthesize_lib_rs(&artifacts.kernel_index),
                );
                artifacts.rust_files.insert(
                    "dispatch.rs".to_string(),
                    synthesize_dispatch(&artifacts.kernel_index),
                );

                // Post-process schedule.rs: append Scoring + Movement
                // SCHEDULE entries before the closing `];` of the
                // SCHEDULE constant. Scoring must run before Movement
                // (Movement reads scoring's action_buf output).
                let sched_key = "schedule.rs".to_string();
                if let Some(sched) = artifacts.rust_files.get_mut(&sched_key) {
                    let extra = "    DispatchOp::Kernel(KernelId::Scoring),\n    \
                                 DispatchOp::Kernel(KernelId::Movement),\n";
                    if let Some(close_idx) = sched.rfind("];\n") {
                        sched.insert_str(close_idx, extra);
                    } else if let Some(stub_idx) = sched.find(
                        "pub const SCHEDULE: &[DispatchOp] = &[];\n",
                    ) {
                        // Defensive: empty-schedule case (shouldn't
                        // happen with the real CG output, but covered
                        // for the empty-Compilation test fixture).
                        ...
                        eprintln!(
                            "compile-dsl: --cg-canonical: scoring/movement splice \
                             could not locate SCHEDULE close marker in schedule.rs",
                        );
                        return ExitCode::FAILURE;
                    }
                } else {
                    eprintln!(
                        "compile-dsl: --cg-canonical: scoring/movement splice \
                         expected schedule.rs in artifacts (none found)",
                    );
                    return ExitCode::FAILURE;
                }
            }
