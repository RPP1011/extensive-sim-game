    ...
    if args.check {
        ...
        // ----- Per-kernel emit block: FusedMaskKernel + MaskUnpackKernel (Task 4).
        {
            ...
            let emittable_masks: Vec<dsl_compiler::ir::MaskIR> = combined
                ...
                .cloned()
                .collect();
            // Phase 0.5 (physics-wgsl-runtime, 2026-04-28): the
            // emit_masks_wgsl_fused emitter produces a 10+ binding
            // WGSL body but the emitted-Rust BGL declares 4 slots —
            // pipeline creation panics on the mismatch. Stub the WGSL
            // to a body that references NO bindings (legal under any
            // BGL) until physics-wgsl-runtime Phase 1 emits a body
            // whose binding declarations match the BGL.
            let _ = emittable_masks; // emitter call deferred
            let fused_wgsl_body = "@compute @workgroup_size(64) fn cs_fused_masks(@builtin(global_invocation_id) gid: vec3<u32>) {}\n".to_string();
            let wgsl = format!(
...
fn cs_fused_agent_unpack(@builtin(global_invocation_id) gid: vec3<u32>) {\n\
    ...
    let _c = cfg.agent_cap;\n\
        ...
        // ----- Per-kernel emit block: ScoringKernel + ScoringUnpackKernel (Task 6).
        {
            ...
            // Phase 0.5 (physics-wgsl-runtime, 2026-04-28): the
            // emit_scoring_wgsl_atomic_views emitter produces 23
            // bindings; the dynamically-built BGL declares ~21 (drift
            // varies by view count). Stub the WGSL to reference NO
            // bindings (legal under any BGL) until physics-wgsl-runtime
            // emits a reconciled scoring body whose bindings exactly
            // match the BGL.
            let _ = &wgsl_specs; // emitter call deferred
            let scoring_wgsl_body = "@compute @workgroup_size(64) fn cs_scoring(@builtin(global_invocation_id) gid: vec3<u32>) {}\n".to_string();
