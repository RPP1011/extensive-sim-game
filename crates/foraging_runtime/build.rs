//! `foraging_runtime` build script. Mirrors
//! `crowd_navigation_runtime/build.rs` verbatim except for the input
//! fixture path (`foraging_colony.sim`).
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/foraging_runtime");
    let sim_path = workspace_root.join("assets/sim/foraging_colony.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse foraging_colony.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve foraging_colony.sim");
    let cg = dsl_compiler::cg::lower::lower_compilation_to_cg(&comp)
        .expect("lower foraging_colony.sim to CG");
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit foraging CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[foraging emit-stats] {} kernels, schedule has {} stages",
    ...
    for (kernel_name, key) in artifacts.kernel_index.iter().zip(kernel_keys.iter()) {
        ...
        println!(
            "cargo:warning=[foraging emit-stats]   {kernel_name}: {bytes} B, {lines} lines, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by foraging_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/foraging_colony.sim and rebuilding.\n\n",
    );
