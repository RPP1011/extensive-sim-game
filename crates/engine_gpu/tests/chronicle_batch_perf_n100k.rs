//! Batch-path perf test at N=100,000 agents.
//!
//! The sync path at N=100k (measured in `perf_n100.rs`) runs ~6.25
...
//!
//! GPU-only — no CPU parity comparison (CPU at N=100k takes hours per
...
//! of wall clock (mostly the sync warmup tick at N=100k plus the
...
fn chronicle_batch_perf_n100k() {
    let mut gpu = match GpuBackend::new() {
        Ok(g) => g,
        Err(e) => {
            eprintln!("chronicle_batch_perf_n100k: GPU init failed — skipping ({e})");
            return;
        }
    };
    ...
    eprintln!("chronicle_batch_perf_n100k: N={N_AGENTS} spawn={spawn_ms}ms backend={}", gpu.backend_label());

    // Warmup sync step: pipeline compile + resident init + chronicle
    // ring alloc. This is expensive at N=100k (~5-6 sec on 4090) but
    ...
    let batch_us_per_tick = batch_elapsed.as_micros() / TICKS as u128;
    eprintln!(
    ...
    // emit records at N=100k if it does at N=20k. If this fails, the
