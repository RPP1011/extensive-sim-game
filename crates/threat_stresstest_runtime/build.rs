//! `threat_stresstest_runtime` build script. Mirrors
//! `cooldown_probe_runtime/build.rs` shape verbatim except for the
//! input fixture path.
//!
//! Lowers `assets/sim/threat_stresstest.sim` through the DSL
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/threat_stresstest_runtime");
    let sim_path = workspace_root.join("assets/sim/threat_stresstest.sim");
    ...
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        ...
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[threat_stresstest lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit threat_stresstest CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[threat_stresstest emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[threat_stresstest emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by threat_stresstest_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/threat_stresstest.sim and rebuilding.\n\n",
    );
