//! `crowd_navigation_runtime` build script. Mirrors
//! `particle_collision_runtime/build.rs` verbatim except for the
//! input fixture path (`crowd_navigation_min.sim`).
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/crowd_navigation_runtime");
    let sim_path = workspace_root.join("assets/sim/crowd_navigation_min.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse crowd_navigation_min.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve crowd_navigation_min.sim");
    let cg = dsl_compiler::cg::lower::lower_compilation_to_cg(&comp)
        .expect("lower crowd_navigation_min.sim to CG");
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit crowd_navigation CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[crowd_navigation emit-stats] {} kernels, schedule has {} stages",
    ...
    for (kernel_name, key) in artifacts.kernel_index.iter().zip(kernel_keys.iter()) {
        ...
        println!(
            "cargo:warning=[crowd_navigation emit-stats]   {kernel_name}: {bytes} B, {lines} lines, {bindings} bindings, {inline_consts} inline_consts",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by crowd_navigation_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/crowd_navigation_min.sim and rebuilding.\n\n",
    );
