//! `abilities_runtime` build script. Mirrors `verb_probe_runtime`'s
//! build.rs verbatim except for the input fixture path.
//!
//! Lowers `assets/sim/abilities_probe.sim` through the DSL compiler
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics: the abilities probe surfaces a
//! `well_formedness` cycle warning between the two chronicle
//! physics rules (op#4 + op#5) — see Gap #2 in the discovery doc.
//! We log the warning and proceed with the partial program so the
//! emit pass still produces kernels for the parts that did lower
//! cleanly.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/abilities_runtime");
    let sim_path = workspace_root.join("assets/sim/abilities_probe.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse abilities_probe.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve abilities_probe.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[abilities lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit abilities_probe CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[abilities emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[abilities emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by abilities_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/abilities_probe.sim and rebuilding.\n\n",
    );
