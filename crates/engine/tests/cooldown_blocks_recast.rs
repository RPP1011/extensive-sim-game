...
use engine::ability::{
    evaluate_cast_gate, AbilityId, AbilityProgram, AbilityRegistry,
    AbilityRegistryBuilder, EffectOp, Gate,
};
...
fn cooldown_blocks_recast_until_next_ready_tick() {
    ...
    let registry: Arc<AbilityRegistry> = Arc::new(b.build());
    ...
    cascade.register_cast_handler(registry.clone());

    let backend = GatedCastBackend {
