//! Task 167 — fear-spread rout mechanic.
//!
...
//! row stacks +0.5 when the observer's total decayed kin_fear crosses
//! 0.3 (via wildcard-slot sum). Result: surviving packmates rout for the
//! ~15-tick half-life window.
//!
...
/// above the 0.3 threshold the Flee row gates on.
#[test]
fn one_fear_spread_crosses_threshold() {
    ...
    // One packmate death → one fold → value = +1.0, above 0.3 gate.
    prime_kin_fear(&mut state, w2, w1);
    let total = state.views.kin_fear.sum_for_first(w2, state.tick);
    assert!(
        total > 0.3,
        "kin_fear {} should cross 0.3 gate after one death",
        total,
    );
    assert!(
        (total - 1.0).abs() < 1e-4,
...
/// The Flee row gains +0.5 on the kin_fear modifier once the sum crosses
/// 0.3 — the key behavioral pin for the rout mechanic. Also confirms the
...
fn flee_score_gains_kin_fear_bonus() {
    ...
    // Prime kin_fear: one dead packmate → sum = 1.0 > 0.3 → +0.6.
    prime_kin_fear(&mut state, w2, w1);
    let s1 = score_row_for(entry, &state, w2, None);
    assert!(
        s1 > s0 + 0.5,
        "Flee should gain ≥+0.5 from kin_fear modifier (+0.6 expected, some epsilon); got {s0} → {s1}",
    );
    assert!(
        (s1 - 0.6).abs() < 1e-3,
        "post-fear Flee = {s1}, expected ≈0.6 (base 0.0 + kin_fear gate +0.6)",
    );
}

/// With kin_fear > 0.3, a healthy wolf with a healthy enemy in range
/// picks Flee over Attack — the intended rout behaviour. Without
/// kin_fear, Attack wins (the wolf would engage). This is the
/// primary behavioral assertion the task calls for.
#[test]
fn wolves_rout_when_packmate_dies() {
    ...
    // Post-death: kin_fear > 0.3, Flee gains +0.6. Attack stays at
    // 0.5 (the Attack row has no kin_fear modifier; its full-hp
    // commit bonus is +0.5). Flee (0.6) > Attack (0.5) — the rout.
    ...
    assert!(
        ...
        "post-death: Flee ({flee_post}) should beat Attack ({attack_post}) — \
         kin_fear > 0.3 adds +0.6 to Flee, routing the survivor",
    );
    assert!(
        flee_post > flee_pre + 0.5,
        "kin_fear should bump Flee by ~+0.6 ({flee_pre} → {flee_post})",
    );
    assert_eq!(
        attack_post, attack_pre,
        "Attack row has no kin_fear modifier — score should be unchanged",
    );
}

/// Decay: after enough ticks (~2 half-lives ≈ 30), kin_fear drops below
/// 0.3 and the Flee bonus vanishes. Proves the rout is transient — the
/// wolf fights again once the fear wears off.
#[test]
fn kin_fear_decays_below_threshold() {
    ...
    assert!(immediate > 0.3, "immediate kin_fear {immediate} should be > 0.3");

    // Advance the observer's observed tick. @decay is `rate=0.955`, so
    // 30 ticks gives 0.955^30 ≈ 0.253 — below the 0.3 gate. 40 ticks
    // gives 0.955^40 ≈ 0.159 — well below. We check at 40 to have
    // margin against tick-boundary math.
    let decayed = state.views.kin_fear.sum_for_first(w2, state.tick + 40);
    assert!(
        decayed < 0.3,
        "after 40 ticks kin_fear {decayed} should decay below 0.3 (rout window elapsed)",
    );

    // And at that tick Flee loses the +0.5 modifier. Clone the fold
...
fn humans_rout_when_packmate_dies_same_mechanic() {
    ...
    assert!(
        total > 0.3,
        "human kin_fear {total} should cross 0.3 gate",
    );
    ...
    assert!(
        (s - 0.6).abs() < 1e-3,
        "human Flee post-death = {s}, expected ≈0.6 (same +0.6 as wolves)",
    );
}
