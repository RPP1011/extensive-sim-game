...
//! favour of the generated `views.memory` ring. The tests below
//! exercise the new API surface: `Memory::push` / `Memory::entries` /
//! `Memory::cursor`.

use engine_rules::views::memory::MemoryEntry;
...
fn spawn_defaults_memory_to_empty() {
    ...
    assert_eq!(views.memory.cursor(a), 0);
}
...
fn push_and_read_memory_events() {
    ...
    views.memory.push(a.raw(), entry);
    views.memory.push(a.raw(), entry);
    assert_eq!(views.memory.cursor(a), 2);
    let row = views.memory.entries(a).expect("owner row present");
...
fn memory_does_not_auto_clear_on_respawn_into_same_slot() {
    ...
    assert_eq!(views.memory.cursor(a), 1);
    ...
    assert_eq!(views.memory.cursor(b), 1);
}
