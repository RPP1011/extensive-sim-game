...
fn eval_view(
    ...
    match pred.field_id {
        ...
        PredicateDescriptor::VIEW_ID_RALLY_BOOST => {
            let a = resolve(slot0).unwrap_or(agent);
            if slot1 == PredicateDescriptor::ARG_WILDCARD {
                views.rally_boost.sum_for_first(a, state.tick)
            } else {
                let b = resolve(slot1).unwrap_or(agent);
                views.rally_boost.get(a, b, state.tick)
            }
        }
        _ => f32::NAN,
    }
...
/// folded.
fn prime_rally_boost(state: &SimState, views: &mut ViewRegistry, observer: AgentId, wounded_kin: AgentId) {
    let ev = Event::RallyCall {
        observer,
        wounded_kin,
        tick: state.tick,
    };
    views.rally_boost.fold_event(&ev, state.tick);
}
...
fn one_rally_call_crosses_threshold() {
    ...
    assert_eq!(
        views.rally_boost.sum_for_first(h2, state.tick),
        0.0,
        "baseline rally_boost should be 0",
    );

    // One kin wound → one fold → value = +1.0, well above 0.3 gate.
    prime_rally_boost(&state, &mut views, h2, h1);
    let total = views.rally_boost.sum_for_first(h2, state.tick);
    assert!(
        total > 0.3,
...
fn humans_rally_when_kin_wounded() {
    ...
    // 0.5 fresh-self = 0.5.
    let h2_pre = score_row_for(entry, &state, &views, h2, Some(wolf));
    let h3_pre = score_row_for(entry, &state, &views, h3, Some(wolf));
    assert!(
        (h2_pre - 0.5).abs() < 1e-4,
    ...
    prime_rally_boost(&state, &mut views, h2, h1);
    prime_rally_boost(&state, &mut views, h3, h1);

    // Post-rally Attack scores. Rally bump is +0.3 on top of the
    // 0.5 baseline. Expected: 0.8.
    let h2_post = score_row_for(entry, &state, &views, h2, Some(wolf));
    let h3_post = score_row_for(entry, &state, &views, h3, Some(wolf));
    assert!(
        h2_post > h2_pre + 0.25,
...
fn rally_boost_decays_below_threshold() {
    ...
    prime_rally_boost(&state, &mut views, h2, h1);
    let immediate = views.rally_boost.sum_for_first(h2, state.tick);
    assert!(immediate > 0.3, "immediate rally_boost {immediate} should be > 0.3");

    // 20 ticks × 0.891 ≈ 0.1 — well below the 0.3 gate.
    let decayed = views.rally_boost.sum_for_first(h2, state.tick + 20);
    assert!(
        decayed < 0.3,
...
fn wolves_rally_when_packmate_wounded_same_mechanic() {
    ...
    prime_rally_boost(&state, &mut views, w2, w1);
...
fn pipeline_wound_triggers_rally() {
    ...
    engine_rules::physics::rally_on_wound::rally_on_wound(
        wolf,
        h1,
        &mut state,
        &mut events,
