//! `megaswarm_1000_runtime` build script. Mirrors
//! `mass_battle_100v100_runtime/build.rs` shape verbatim except for
//! the input fixture path.
//!
//! Lowers `assets/sim/megaswarm_1000.sim` through the DSL compiler
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/megaswarm_1000_runtime");
    let sim_path = workspace_root.join("assets/sim/megaswarm_1000.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse megaswarm_1000.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve megaswarm_1000.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[megaswarm_1000 lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit megaswarm_1000 CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[megaswarm_1000 emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[megaswarm_1000 emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by megaswarm_1000_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/megaswarm_1000.sim and rebuilding.\n\n",
    );
