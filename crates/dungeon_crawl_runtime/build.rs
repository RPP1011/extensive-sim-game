//! `dungeon_crawl_runtime` build script. Mirrors `duel_1v1_runtime`'s
//! build.rs verbatim except for the input fixture path.
//!
//! Lowers `assets/sim/dungeon_crawl.sim` through the DSL compiler
//! pipeline (parse -> resolve -> CG lower -> schedule -> emit). The
//! resulting WGSL + Rust files land in OUT_DIR/<kernel>.{wgsl,rs}
//! and are concatenated into `OUT_DIR/generated.rs`.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/dungeon_crawl_runtime");
    let sim_path = workspace_root.join("assets/sim/dungeon_crawl.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse dungeon_crawl.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve dungeon_crawl.sim");
    // Tolerate lower diagnostics — same well-formed P6 + cycle warnings
    // as duel_1v1 (chronicle physics writes agent.hp/alive + Spell
    // scoring reads agent.hp on the same ring).
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[dungeon_crawl lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit dungeon_crawl CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[dungeon_crawl emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[dungeon_crawl emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by dungeon_crawl_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/dungeon_crawl.sim and rebuilding.\n\n",
    );
