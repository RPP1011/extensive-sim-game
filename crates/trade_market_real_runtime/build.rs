//! `trade_market_real_runtime` build script. Mirrors
//! `duel_1v1_runtime`'s build.rs shape verbatim except for the
//! input fixture path. Tolerates lower diagnostics — the cycle
//! warning (Trade event read by ApplyTrade writes hp + Buy verb
//! scoring reads hp) is advisory not blocking.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/trade_market_real_runtime");
    let sim_path = workspace_root.join("assets/sim/trade_market_real.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse trade_market_real.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve trade_market_real.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[trade_market_real lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit trade_market_real CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[trade_market_real emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[trade_market_real emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by trade_market_real_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/trade_market_real.sim and rebuilding.\n\n",
    );
