//! `predator_prey_real_runtime` build script. Mirrors
//! `foraging_real_runtime`'s build.rs shape verbatim except for the
//! input fixture path.
//!
//! Lowers `assets/sim/predator_prey_real.sim` through the DSL compiler
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics — predator_prey_real inherits the
//! duel_1v1 P6 + cycle warnings (chronicle physics writes
//! agent.{hp,alive,hunger} on the same ring scoring/mask might read;
//! the well_formed checker doesn't yet model @phase(post) authored
//! mutation as legitimate).
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/predator_prey_real_runtime");
    let sim_path = workspace_root.join("assets/sim/predator_prey_real.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse predator_prey_real.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve predator_prey_real.sim");
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[predator_prey_real lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit predator_prey_real CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[predator_prey_real emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[predator_prey_real emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by predator_prey_real_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/predator_prey_real.sim and rebuilding.\n\n",
    );
