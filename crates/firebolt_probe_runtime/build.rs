//! `firebolt_probe_runtime` build script — Plan G G4.
//!
//! Lowers `assets/sim/firebolt_probe.sim` through the DSL compiler
...
//! into `src/lib.rs`.
//!
//! Tolerates lower diagnostics — the consumer rule
//! (`ApplyChronicleDamage`) trips the same P6 well_formed diagnostic
//! that `apply_ability_chronicle_consumer.sim` trips (PerEvent +
//! `agents.set_hp` is flagged because P6's strict reading is "events
//! are the mutation channel"). Mirrors
//! `apply_ability_chronicle_consumer_runtime/build.rs`'s
//! `DriverOutcome::Err → o.program` pattern: surface the diag as a
//! cargo:warning, but keep the emitted kernels.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/firebolt_probe_runtime");
    let sim_path = workspace_root.join("assets/sim/firebolt_probe.sim");
    ...
    // Tolerate lower diagnostics — the ApplyChronicleDamage consumer
    // rule trips P6 (PerEvent + agents.set_hp). Mirror
    // `apply_ability_chronicle_consumer_runtime/build.rs`: surface the
    // diag, take `o.program` anyway.
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[firebolt_probe lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit firebolt_probe CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[firebolt_probe emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[firebolt_probe emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by firebolt_probe_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/firebolt_probe.sim and rebuilding.\n\n",
    );
