//! `predator_prey_runtime` build script. Mirrors `boids_runtime`'s
//! build.rs verbatim except for the input fixture path.
//!
//! Tracks `assets/sim/predator_prey_min.sim` (the staged working file)
//! rather than `predator_prey.sim` (the design target). Each PP stage
//! grows `predator_prey_min.sim` toward parity with `predator_prey.sim`
//! — when they match, we collapse the two and point this build.rs at
//! `predator_prey.sim` directly.
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/predator_prey_runtime");
    let sim_path = workspace_root.join("assets/sim/predator_prey_min.sim");
    ...
    let cg = dsl_compiler::cg::lower::lower_compilation_to_cg(&comp)
        .expect("lower predator_prey_min.sim to CG");
    ...
    println!(
        "cargo:warning=[predator_prey emit-stats] {} kernels, schedule has {} stages",
    ...
    for (kernel_name, key) in artifacts.kernel_index.iter().zip(kernel_keys.iter()) {
        ...
        println!(
            "cargo:warning=[predator_prey emit-stats]   {kernel_name}: {bytes} B, {lines} lines, {bindings} bindings, {inline_consts} inline_consts",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by predator_prey_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/predator_prey_min.sim and rebuilding.\n\n",
    );
