//! `quest_probe_runtime` build script. Mirrors `cooldown_probe_runtime`'s
//! build.rs shape verbatim except for the input fixture path.
//!
//! Lowers `assets/sim/quest_probe.sim` through the DSL compiler
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/quest_probe_runtime");
    let sim_path = workspace_root.join("assets/sim/quest_probe.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse quest_probe.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve quest_probe.sim");
    ...
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        ...
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[quest_probe lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit quest_probe CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[quest_probe emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[quest_probe emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by quest_probe_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/quest_probe.sim and rebuilding.\n\n",
    );
