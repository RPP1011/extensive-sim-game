//! `auction_runtime` build script. Mirrors
//! `foraging_runtime/build.rs` verbatim except for the input fixture
//! path (`auction_market.sim`).
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/auction_runtime");
    let sim_path = workspace_root.join("assets/sim/auction_market.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse auction_market.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve auction_market.sim");
    let cg = dsl_compiler::cg::lower::lower_compilation_to_cg(&comp)
        .expect("lower auction_market.sim to CG");
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit auction CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[auction emit-stats] {} kernels, schedule has {} stages",
    ...
    for (kernel_name, key) in artifacts.kernel_index.iter().zip(kernel_keys.iter()) {
        ...
        println!(
            "cargo:warning=[auction emit-stats]   {kernel_name}: {bytes} B, {lines} lines, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by auction_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/auction_market.sim and rebuilding.\n\n",
    );
