//! `apply_ability_verb_smoke_runtime` build script. Mirrors
//! `apply_ability_smoke_runtime`'s build.rs verbatim except for the
//! input fixture path.
//!
//! Lowers `assets/sim/apply_ability_verb_smoke.sim` through the DSL
...
fn main() {
    ...
    let workspace_root = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above crates/apply_ability_verb_smoke_runtime");
    let sim_path = workspace_root.join("assets/sim/apply_ability_verb_smoke.sim");
    ...
    let program = dsl_compiler::parse(&src).expect("parse apply_ability_verb_smoke.sim");
    let comp = dsl_ast::resolve::resolve(program).expect("resolve apply_ability_verb_smoke.sim");
    // Tolerate lower diagnostics — surface what's emitted even if some
    // downstream rules failed to lower (mirrors apply_ability_smoke's pattern).
    let cg = match dsl_compiler::cg::lower::lower_compilation_to_cg(&comp) {
        Ok(p) => p,
        Err(o) => {
            for d in &o.diagnostics {
                println!("cargo:warning=[apply_ability_verb_smoke lower diag] {d}");
            }
            o.program
        }
    };
    ...
    let artifacts = dsl_compiler::cg::emit::emit_cg_program(&schedule_result.schedule, &cg)
        .expect("emit apply_ability_verb_smoke CG program");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));

    println!(
        "cargo:warning=[apply_ability_verb_smoke emit-stats] {} kernels, schedule has {} stages",
    ...
    for kernel_name in &artifacts.kernel_index {
        ...
        println!(
            "cargo:warning=[apply_ability_verb_smoke emit-stats]   {kernel_name}: {bytes} B, {bindings} bindings",
        );
    }
    ...
    generated.push_str(
        "// AUTO-CONCATENATED from compiler-emitted artifacts by apply_ability_verb_smoke_runtime/build.rs.\n\
         // Do not edit. Regenerate by editing assets/sim/apply_ability_verb_smoke.sim and rebuilding.\n\n",
    );
