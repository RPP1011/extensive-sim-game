...
pub(crate) fn apply_random_policy(
    sim: &game::ai::core::SimState,
    unit: &game::ai::core::UnitState,
    uid: u32,
    mask_arr: &[bool],
    mask_vec: &[bool],
    ...
    tick: u64,
    rng: &mut u64,
    intents: &mut Vec<game::ai::core::UnitIntent>,
    steps: &mut Vec<RlStep>,
    vis_map: Option<&game::ai::goap::spatial::VisibilityMap>,
    nav: Option<&game::ai::pathing::GridNav>,
) {
    use game::ai::core::{Team, UnitIntent};
    use game::ai::core::ability_eval::{extract_game_state_v2, extract_game_state_v2_spatial};
        ...
        let combat_intent = if combat_type == COMBAT_TYPE_HOLD {
            ...
            if let Some(target) = sim.units.iter()
                ...
                game::ai::core::IntentAction::UseAbility {
                    ...
                    target: game::ai::effects::AbilityTarget::Unit(target.id),
                }
            } else {
                game::ai::core::IntentAction::Hold
            }
...
pub(crate) fn apply_v5_policy(
    ac: &game::ai::core::ability_transformer::ActorCriticWeightsV5,
    sim: &game::ai::core::SimState,
    unit: &game::ai::core::UnitState,
    uid: u32,
    gs_v2: &game::ai::core::ability_eval::GameStateV2,
    ...
    let (target_idx, target_lp, combat_intent) = match combat_type {
        ...
        t if t == COMBAT_TYPE_HOLD => {
            (0, 0.0, game::ai::core::IntentAction::Hold)
        }
        t @ 2..=9 => {
            let ab_idx = t - 2;
        ...
        _ => (0, 0.0, game::ai::core::IntentAction::Hold),
    };
...
pub(crate) fn check_drill_objective(
    obj: Option<&game::scenario::ObjectiveDef>,
    sim: &game::ai::core::SimState,
    heroes_alive: usize,
    enemies_alive: usize,
) -> Option<(String, f32)> {
    ...
    match obj.objective_type.as_str() {
        "reach_position" => {
            if let (Some(target), Some(radius)) = (obj.position, obj.radius) {
                for unit in sim.units.iter().filter(|u| u.team == game::ai::core::Team::Hero && u.hp > 0) {
        ...
        "reach_entity" => {
            ...
            for hero in sim.units.iter().filter(|u| u.team == game::ai::core::Team::Hero && u.hp > 0) {
                for enemy in sim.units.iter().filter(|u| u.team == game::ai::core::Team::Enemy && u.hp > 0) {
...
#[allow(dead_code)]
pub(crate) fn hp_fraction(sim: &game::ai::core::SimState, team: game::ai::core::Team) -> f32 {
    let mut lost = 0i32;
    let mut total = 0i32;
    for u in &sim.units {
        if u.team == team {
