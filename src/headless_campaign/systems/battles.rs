...
pub fn tick_battles(
    ...
    if combat_mode == CombatMode::TacticalSim {
        ...
        let results: Vec<(usize, crate::headless_campaign::combat_oracle::tactical_bridge::TacticalResult)> = to_resolve
            .into_iter()
            .map(|(idx, party_id, enemy_strength, seed)| {
                let members: Vec<&crate::headless_campaign::state::Adventurer> = state
                    .adventurers
                    .iter()
                    .filter(|a| a.party_id == Some(party_id))
                    .collect();
                let result = crate::headless_campaign::combat_oracle::tactical_bridge::run_tactical_combat(
                    &members,
                    enemy_strength,
                    seed,
                    state,
