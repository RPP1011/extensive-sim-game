...
pub fn tick_faction_ai(
    ...
    for fi in 0..n_factions {
        ...
        let relation = state.factions[fi].relationship_to_guild;
        let is_coalition = state.factions[fi].coalition_member;
...
fn faction_at_war(
    ...
    cfg: &crate::headless_campaign::config::FactionAiConfig,
) -> String {
    let strength = state.factions[fi].military_strength;
