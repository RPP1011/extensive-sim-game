...
| G2 | **PerEvent physics rule + `self` → undefined `agent_id`.** A physics rule that is not `@phase(per_agent)` dispatches PerEvent; its kernel preamble binds `event_idx`, not `agent_id`. Referencing `self` emits WGSL reading an undeclared `agent_id` → naga panic at first `step()`. | S2 | **CLOSED** (7d02f20c) | Now a typed `CgError::SelfRefInPerEventBody` at lower time (read-side mirror of the P6 self-write check). Test: `self_in_per_event_rejected.rs`. Surfaced + fixed one latent test-fixture bug. |
...
1. **G2 — PerEvent physics rule + `self` → typed compile error.** ✅ **DONE** (7d02f20c). Lowest-risk silent→loud conversion: a body walk rejects `self` references in a PerEvent physics rule with a diagnostic pointing at `@phase(per_agent)`. Gate: `self_in_per_event_rejected.rs`.
