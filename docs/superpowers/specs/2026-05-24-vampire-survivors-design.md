...
| 4 | Spawn / infinite ramp | **Not exercised this slice (deferred)** — known gap; runtime-owned today | DSL `summon`/spawn primitive |
| 5 | Entity-subkind scalar accumulator field (`level`) — leveling fallback B | **Not exercised this slice (deferred)** — fallback B not needed; probe A (floor) passed | — |

**Findings summary.** The VS combat + power-ramp loop compiles cleanly today: mana-band-gated kiting, both auto-fire weapons, XP-on-kill fold, discrete leveling via `floor`, and fixed-priority runtime-varying upgrade selection all lower without workaround. The one genuine new correctness gap uncovered is the silently-dropped fold `where`-guard (#3c): `FoldHandlerIR` carries no `where_clause` field, so a guard like `where k == 0u` on a payload field is accepted by the parser and AST but discarded by the resolver with no diagnostic — making per-kind tally-by-fold-guard impossible until the IR-plumbing is wired (`where_clause` on `FoldHandlerIR` + a guard arm in `lower_one_handler`; this is a pure IR fix, not a parser change). The only other wall is cosmetic: `else if` sugar fails to parse, but the semantically equivalent nested `else { if … }` form lowers fine.

**Landed shape (Task 8).** `enum UpgradeKind` declared (3a PASS, unused). One `event UpgradeChosen { player, kind: u32 }`, one `view upgrades_total(player)` fold (un-split because the per-kind `where` guard walls — 3c), and a `physics ChooseUpgrade` whose body is a nested value-returning `if/else` over view-gated conditions selecting a `u32` kind (3 + 3b PASS). Net: **fixed-priority, runtime-varying upgrade selection IS expressible today**; the only true walls are the `else if` parser sugar (cosmetic — nest manually) and the fold `where`-guard drop (substantive — blocks per-kind tally-by-fold-guard; fix is pure IR-plumbing since the AST+parser already carry the guard).
